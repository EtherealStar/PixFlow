...
public class CustomDownloadService {
  ...
  public DownloadHandle build(CustomBundleRequest request) {
    ...
    String archiveKey =
        "custom-downloads/" + UUID.randomUUID() + "/" + archiveName(request.archiveName());
    ...
    return new DownloadHandle(
        url,
        clock.instant().plus(taskProperties.getDownload().getSingleUrlExpiry()),
        "application/zip",
        ref.size());
  }
  ...
  private DownloadBundleBuilder.BundleSource resolve(CustomBundleRequest.BundleItem item) {
    ...
    if ("ASSET_IMAGE".equals(type)) {
      ...
      if (image.location() == null) {
        throw new PixFlowException(
            TaskErrorCode.TASK_DOWNLOAD_NOT_READY, "asset image is not ready for bundle");
      }
    ...
    if ("TASK_RESULT".equals(type)) {
      ...
      return new DownloadBundleBuilder.BundleSource(
          firstNonBlank(item.filename(), result.getDisplayName(), result.getId() + ".image"),
          content.location());
    }
    throw new PixFlowException(
        TaskErrorCode.TASK_DOWNLOAD_NOT_READY, "unknown download bundle item type: " + item.type());
  }
  ...
  private static long parseId(String id) {
    try {
      ...
      throw new PixFlowException(
          TaskErrorCode.TASK_DOWNLOAD_NOT_READY, "invalid download bundle item id", ex);
    }
  }

  private static String archiveName(String name) {
    String normalized =
        firstNonBlank(name, "selected-images.zip").replaceAll("[^A-Za-z0-9_.-]", "_");
