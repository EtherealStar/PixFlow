...
public interface RubricsRunItemMapper extends BaseMapper<RubricsRunItemEntity> {
    ...
    @Select("""
            ...
            order by id asc
            """)
    List<RubricsRunItemEntity> findRetryableByRunId(@Param("runId") long runId);
    ...
    @Update("""
            ...
            where id = #{itemId}
            """)
    int markRunning(@Param("itemId") long itemId, @Param("status") RunItemStatus status, @Param("now") Instant now);
