...
public class ResizeOp implements ImageOp {
    ...
    public RasterImage apply(RasterImage src) {
        try {
            ...
            BufferedImage resized = switch (spec.mode()) {
                case EXACT -> Thumbnails.of(src.buffer()).forceSize(dims.width, dims.height).asBufferedImage();
                case FIT -> Thumbnails.of(src.buffer()).size(dims.width, dims.height).keepAspectRatio(true).asBufferedImage();
                case FILL -> fill(src, dims.width, dims.height);
            };
            return RasterImage.of(resized, src.sourceFormat());
    ...
    private BufferedImage fill(RasterImage src, int targetWidth, int targetHeight) throws IOException {
        ...
        BufferedImage scaled = Thumbnails.of(src.buffer()).forceSize(scaledWidth, scaledHeight).asBufferedImage();
