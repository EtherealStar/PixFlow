...
@SpringBootTest(classes = OrphanedResultObjectIntegrationTest.TestApp.class, properties = {
...
class OrphanedResultObjectIntegrationTest {
    ...
    static final MySQLContainer<?> MYSQL = new MySQLContainer<>("mysql:8.4")
            .withDatabaseName("pixflow_task_orphan").withUsername("pixflow").withPassword("pixflow");
    ...
    void objectWrittenByStaleEpochRemainsUnreferencedAndInvisible() {
        ...
        String objectKey = "results/" + task.getId() + "/units/" + UnitKeyCodec.sha256(unitKey)
                + "/epochs/1/output.png";
        ObjectLocation orphan = ObjectLocation.of(BucketType.RESULTS, objectKey);
        ...
        WorkUnit unit = new WorkUnit(task.getId().toString(), TaskType.IMAGE_PROCESS,
                unitKey, List.of(), null, null);
        WorkUnitCompletion completion = new WorkUnitCompletion.Succeeded(
                unit, 1, now, now.plusSeconds(1), objectKey, null, 3L, List.of());
        var repository = new WorkUnitResultRepository(
                resultMapper, memberMapper, taskMapper, objectStorage, new ObjectMapper());

        assertThat(repository.commit(new ExecutionRun(task.getId().toString(), 1, () -> true), completion))
