...
class ProcessWorkerPlanTest {
    ...
    void rebuildsOnlyFrozenDerivedRetrySubsetFromFullCanonicalPlan() throws Exception {
        ...
        ImageDescriptor image = ImageDescriptor.single("11", "SKU-11", "7/11.png");
        ...
        WorkUnitSelection retrySelection = new WorkUnitSelection(List.of(
                new WorkUnitSelection.Item(UnitKind.BRANCH, "11", "branch-b", List.of(image))));
        ProcessWorker worker = new ProcessWorker(objectMapper, new DagJsonReader(objectMapper), dagFacade,
                expander, null, null, null, null, null);

        var units = worker.plan("99", 7L,
    ...
    private static ExecutableBranch branch(String branchId, String memberId) {
        return new ExecutableBranch(UnitKind.BRANCH, branchId, memberId,
                List.of(), null, List.of(), ExecutableBranch.EncodeTarget.defaultJpeg());
    }
}
