...
class DerivedRetryTaskTest {
  ...
  void retryFailedCreatesQueuedTaskFromOnlyFailedFrozenUnits() throws Exception {
    ...
    failed.setUnitKey(
        UnitKeyCodec.encode(new UnitKey("10", UnitKind.BRANCH, "image-1", "branch-a")));
    when(results.findByTaskIdAndStatus(10L, ResultStatus.FAILED)).thenReturn(List.of(failed));
    when(tasks.transit(
            101L, TaskStatus.PENDING, TaskStatus.QUEUED, Instant.parse("2026-07-13T03:00:00Z")))
        .thenReturn(1);
    doAnswer(
            invocation -> {
        ...
        .when(tasks)
        .insert(any(ProcessTask.class));

    PendingTaskEnqueuer enqueuer = mock(PendingTaskEnqueuer.class);
    RetryFailedTaskService service =
        new RetryFailedTaskService(
            tasks,
            results,
            new IdempotencyGuard(idempotencyStore, tasks),
            enqueuer,
            new ObjectMapper(),
    ...
    WorkUnitSelection selection =
        new ObjectMapper()
            .readValue(task.getValue().getUnitSelectionJson(), WorkUnitSelection.class);
  ...
  private static ProcessTask sourceTask() throws Exception {
    ...
    source.setUnitSelectionJson(
        new ObjectMapper().writeValueAsString(new WorkUnitSelection(List.of(failed, succeeded))));
    return source;
  }
}
