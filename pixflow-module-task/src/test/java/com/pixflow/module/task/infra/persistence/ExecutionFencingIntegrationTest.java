...
class ExecutionFencingIntegrationTest {
    ...
    void higherEpochFencesOldHeartbeatResultAndTerminalWritesWhileSuccessRemainsImmutable() {
        ...
        assertThat(taskMapper.markTerminalEpoch(task.getId(), 1, TaskStatus.FAILED,
                1, 1, "stale owner", now)).isZero();
        assertThat(resultMapper.selectByUnit(task.getId(), result.getUnitKey()).getStatus())
                .isEqualTo(ResultStatus.SUCCESS);
        assertThat(taskMapper.markTerminalEpoch(task.getId(), 2, TaskStatus.COMPLETED,
                1, 1, null, now)).isEqualTo(1);
    ...
    private static ProcessResult success(Instant now) {
        ...
        row.setOutputMinioKey("results/1/output.png"); row.setAttemptCount(1);
        row.setStartedAt(now); row.setFinishedAt(now); return row;
    }

    private static ProcessResult failed(Instant now) {
        ProcessResult row = new ProcessResult(); row.setStatus(ResultStatus.FAILED);
        row.setFailureCode("FAILED"); row.setAttemptCount(1);
        row.setStartedAt(now); row.setFinishedAt(now); return row;
    }
