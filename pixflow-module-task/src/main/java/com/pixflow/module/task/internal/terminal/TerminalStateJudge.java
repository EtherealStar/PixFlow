...
public class TerminalStateJudge {
  ...
  public TaskStatus judge(long taskId, long runEpoch) {
    ...
    if (resultMapper.countPendingPublication(taskId) > 0) {
      throw new IllegalStateException("仍有 Generated Image 等待发布，任务不能进入终态");
    }
    int total =
        task.getTotalCount() == null
            ? resultMapper.findByTaskId(taskId).size()
            : task.getTotalCount();
    ...
    int updated =
        taskMapper.markTerminalEpoch(
            ...
            terminal == TaskStatus.FAILED ? "all units failed" : null,
            clock.instant());
    if (updated != 1) {
      return taskMapper.selectById(taskId).getStatus();
    }
  ...
  static TaskStatus decide(int total, int success, int failed, int skipped) {
    ...
    if (total > 0 && success == 0 && skipped == total) {
