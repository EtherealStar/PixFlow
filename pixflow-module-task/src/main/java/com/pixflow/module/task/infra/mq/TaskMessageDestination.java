...
public final class TaskMessageDestination {
    ...
    public static MessageDestination destination(TaskProperties properties, String taskId) {
        return MessageDestination.of(properties.getMq().getTopic(), properties.getMq().getTag()).withKey("task:" + taskId);
    }

    public static ConsumerBinding binding(TaskProperties properties) {
        return ConsumerBinding.of(properties.getMq().getTopic(), properties.getMq().getTag(), properties.getMq().getConsumerGroup(), TaskMessage.class);
    }
}
