...
public class RetryFailedTaskService {
    ...
    public RetryFailedTaskService(ProcessTaskMapper taskMapper, ProcessResultMapper resultMapper,
                                  IdempotencyGuard idempotencyGuard, PendingTaskEnqueuer enqueuer,
                                  ObjectMapper objectMapper, Clock clock, TaskEventPublisher eventPublisher) {
    ...
    public RetryTaskResponse retry(RetryFailedTaskCommand command) {
        ...
        return idempotencyGuard.findExistingTaskId(idempotencyKey)
    ...
    private RetryTaskResponse create(long sourceId, String idempotencyKey) {
        ...
        if (source.getStatus() != TaskStatus.PARTIAL && source.getStatus() != TaskStatus.FAILED) {
            throw new PixFlowException(TaskErrorCode.TASK_RETRY_SOURCE_NOT_TERMINAL,
                    "retry source must be PARTIAL or FAILED");
        }
        ...
        var failedItems = sourceSelection.items().stream()
                .filter(item -> failedKeys.contains(UnitKeyCodec.encode(new UnitKey(
                        Long.toString(sourceId), item.kind(), item.memberId(), item.branchId()))))
                .toList();
        if (failedItems.isEmpty()) {
            throw new PixFlowException(TaskErrorCode.TASK_NO_FAILED_UNITS,
                    "retry source has no failed work units");
        }
        ...
        try {
            ...
            return idempotencyGuard.findExistingTaskId(idempotencyKey).map(this::loadResponse)
        ...
        registerAfterCommit(() -> {
            ...
            eventPublisher.publishCreated(new TaskCreatedEvent(derived.getId().toString(), derived.getTaskType(),
                    derived.getConversationId(), derived.getPackageId(), clock.instant()));
        });
        return new RetryTaskResponse(derived.getId().toString(), Long.toString(sourceId),
                failedItems.size(), TaskStatus.QUEUED);
    }
    ...
    private ProcessTask derivedTask(ProcessTask source, String idempotencyKey,
                                    WorkUnitSelection selection, Instant now) {
        ...
        try {
            ...
            throw new PixFlowException(TaskErrorCode.TASK_CREATE_FAILED, "serialize retry selection failed", e);
        }
        return derived;
    }
    ...
    private WorkUnitSelection readSelection(ProcessTask source) {
        try {
            ...
            throw new PixFlowException(TaskErrorCode.TASK_CREATE_FAILED, "deserialize source selection failed", e);
        }
    }
    ...
    private void precreatePendingResults(long taskId, WorkUnitSelection selection, Instant now) {
        for (var item : selection.items()) {
            ...
            result.setUnitKey(UnitKeyCodec.encode(new UnitKey(
                    Long.toString(taskId), item.kind(), item.memberId(), item.branchId())));
    ...
    private RetryTaskResponse loadResponse(String taskId) {
        ...
        return new RetryTaskResponse(taskId, task.getRetryOfTaskId().toString(),
                task.getTotalCount() == null ? 0 : task.getTotalCount(), task.getStatus());
    }
    ...
    private static long parseSourceId(RetryFailedTaskCommand command) {
        try {
            ...
            throw new PixFlowException(TaskErrorCode.TASK_NOT_FOUND,
                    "task not found: " + command.sourceTaskId().value());
        }
    }
