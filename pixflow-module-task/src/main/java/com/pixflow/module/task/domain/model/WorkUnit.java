...
public record WorkUnit(
    ...
    public static WorkUnit branch(String taskId, ExecutableBranch branch, List<ImageDescriptor> images) {
        UnitKey key = branch.kind() == com.pixflow.harness.state.model.UnitKind.GROUP
                ? UnitKey.group(taskId, branch.memberId(), branch.branchId())
                : UnitKey.branch(taskId, branch.memberId(), branch.branchId());
        return new WorkUnit(taskId, TaskType.IMAGE_PROCESS, key,
                images, branch, null);
    }

    public static WorkUnit generative(String taskId, GenerativeUnitSpec spec) {
        return new WorkUnit(taskId, TaskType.IMAGE_GEN, UnitKey.generative(taskId, spec.sourceImageId()),
                List.of(), null, spec);
    }
