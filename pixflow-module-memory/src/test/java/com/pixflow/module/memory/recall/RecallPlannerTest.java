...
class RecallPlannerTest {
    ...
    @Test
    void plansPreferenceEveryTurnButDoesNotTreatPromptTextAsCanonicalSkuIdentity() {
        ...
        assertThat(plan.recallSkuHistory()).isFalse();
        assertThat(plan.recallInsight()).isTrue();
        assertThat(plan.skuIds()).isEmpty();
        assertThat(plan.insightQuery()).contains("连衣裙", "点击率");
    }

    @Test
    void usesResolvedSkuSignalsAndReducesInsightTopNWhenBudgetIsTight() {
        ...
        RecallSignals signals = new RecallSignals(List.of("SKU999"), List.of("家居"), List.of(), List.of());
        RecallPlan plan = new RecallPlanner(new MemoryProperties()).plan(request, signals);
    ...
    private static MemoryContextRequest request() {
        return new MemoryContextRequest("c1", 1, "t1", "", List.of(), List.of(), Map.of(), 1000);
    }
}
