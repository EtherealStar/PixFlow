...
2. **provider-neutral**。loop 只依赖 `infra/ai` 暴露的 provider-neutral 抽象（`ModelClient` 协议、`ModelStreamEvent`、`ProviderError`、`ModelUsage`、`ModelRetryRunner`），不理解任何供应商 wire 协议、不读私有 `stop_reason`、不碰 API key 与模型目录（`model-provider-architecture.md` 一致约束）。
...
6. **错误恢复分层、各司其职**。可重试错误（限流/网络/5xx）由 `infra/ai` 的 `ModelRetryRunner` 缓冲式退避消化，loop 不感知中间失败；loop 只处理需要**跨模块协调**才能恢复的两类——`CONTEXT_LIMIT`（触发 context 反应式压缩）和**输出截断**（抬高 max output / 续写恢复）；不可恢复错误经 `common.ErrorRecorder` 落盘后向上抛（`error-handling-architecture.md` 分层）。
...
| 重试 | `ModelRetryRunner`（services/model） | `infra/ai` 的 `ModelRetryRunner`（loop 调用，不自实现退避） |
...
loop ──► infra/ai（ModelClient 协议 / ModelRetryRunner / ModelStreamEvent / ProviderError / ModelUsage）
...
> **新增依赖边说明（需同步 `module-dependency-dag-plan.md`）**：当前 DAG 已有 `tools→loop`、`hooks→loop`、`context→loop`、`permission→loop`、`eval→loop`、`loop→agent`。但 loop 还需依赖 `infra/ai` 的 provider-neutral 抽象（`ModelClient`/`ModelRetryRunner`/`ModelStreamEvent`），DAG 当前**缺一条 `ai --> loop` 边**，应补上（`infra/ai` 在 Wave 1，loop 在 Wave 4，无环；与 `ai→dag`/`ai→vision` 等同理：均为 infra/ai 能力消费方）。
>
...
public enum TransitionReason {
    ...
    RATE_LIMIT_RETRY,            // ModelRetryRunner 重试同一迭代（loop 仅记录，退避在 infra/ai）
...
- 失败的模型调用 attempt 的 partial delta **不 emit**：`ModelRetryRunner` 缓冲整个 attempt，成功才放行（`model-provider-architecture.md` 缓冲式重试），loop 只 emit 成功 attempt 的事件。
...
- 错误恢复（反应式压缩 / 输出截断）需要「缓冲完一次 attempt 再决定 continue」，阻塞迭代比 Flux 直观得多；参考的 `ModelRetryRunner` 正是「缓冲全部 events，成功才 yield」。
...
| 错误 | 处理层 | loop 行为 |
|---|---|---|
| 限流 / 网络 / 5xx（`retryable=true` 且非 `context_limit`） | `infra/ai.ModelRetryRunner` | 缓冲式指数退避在 infra/ai 内消化；loop 看不到中间失败，仅对应一次 `RATE_LIMIT_RETRY`（retry runner 回调）；耗尽抛 `RetryExhaustedError` 上抛 |
...
- **失败 attempt 的 partial 不泄漏**：`ModelRetryRunner` 丢弃失败 attempt 的全部 events，loop 只 emit 成功 attempt（`model-provider-architecture.md`）。反应式压缩 / escalate 重试本迭代时**不 append assistant**（避免把半截/超窗的 assistant 写进链）。
...
| `RATE_LIMIT_RETRY` | `ModelRetryRunner` 重试前回调 | 重试同一迭代（退避在 infra/ai） | 否 |
...
| `infra/ai` | 依赖 provider-neutral `ModelClient.stream(snapshot)`、`ModelRetryRunner.stream(op, onRetry)`、`ModelStreamEvent`（`messageCompleted.metadata.toolCalls` 续轮信号、`outputInterrupted` 恢复信号）、`ProviderError`（`CONTEXT_LIMIT` 等）、`ModelUsage` |
...
- **重试边界**：mock retry runner 触发 `RATE_LIMIT_RETRY`（loop 仅记录、退避在 infra/ai）；`RetryExhaustedError` 上抛断言不吞、不 emit error 事件。
