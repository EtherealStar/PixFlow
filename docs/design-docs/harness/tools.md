...
1. **两套工具严格分离**。本 Registry 只管 **Agent 级动作**（`search` / `read` / `agent` / `submit_image_plan` / `submit_imagegen_plan` / `plan` / `plan_exit`）。DAG 内部的**像素工具**（`remove_bg` / `resize` / `compose_group` …）由 `module/dag` 执行引擎管理，**绝不进入本 Registry**（`design.md §5.2`）。
2. **工具集零真实副作用、零令牌**。Agent 永远不能扣下真实处理/生成的扳机：所有会产生像素处理、付费 API 调用、生图的动作，都被建模为「提案进确认队列」，由人在带外（前端确认 → REST 端点）拍板执行。因此**本 Registry 的任何工具都不携带、不消费确认令牌**；确认令牌闸门挪到 `module/conversation` 的确认 REST 边界（见 [八.3](#83-hitl-在带外工具层零令牌)）。
...
| 资源模型 | `ToolTarget`（file/directory/glob/command/url）+ guard 路径校验 | **无 ToolTarget、无 guard、无路径模型**；安全靠确认队列（带外）+ permission |
...
| 副作用闸门 | 工具直接执行真实副作用 | **零真实副作用**：会产生后果的动作只能「提案进确认队列」 |
| 令牌 | 无（编码 agent 无 HITL 令牌） | 工具层**零令牌**；令牌闸门在带外确认 REST 边界 |
...
| `compile_dag` + `submit_dag` | `submit_image_plan` | 合并：Agent 直接产出 DAG JSON 作为入参（省掉 `compile_dag` 的二次 LLM 往返）；工具只校验 + 入队，**不执行**；`submit_dag` 作为 Agent 工具消失，执行移到带外确认 |
| `run_vision_subagent` | `agent`（type=vision） | 统一 subagent runner |
| `run_imagegen_subagent` | `submit_imagegen_plan` | 生图不直接执行：工具产出**生图提示词提案**进确认队列，确认后带外执行 |
| —— | `agent`（type=explore） | 新增只读探索子 Agent |
| —— | `plan` / `plan_exit` | 新增 Plan 模式开关（`design.md` 无此概念） |

> 连带的非工具变更（需在 `module/file`、`module/commerce`、数据模型回写）：上传后把同 SKU 图片发 VLLM 抽取商品名/描述、写入 `asset_copy` 作为 `search`/`read` 的数据底座（这是**固定流程**，不是 `agent[vision]` 工具）。本文只定义 tools 边界，数据底座细节属 `module/file` / `module/commerce`。

---

## 三、工具清单（本期）

本 Registry 在本期承载 7 个 Agent 级工具。所有命名为 snake_case，作为 registry key 与 provider-visible function name。
...
| `agent` | 运行只读子 Agent（`type=vision` 看图 / `type=explore` 探索关联 SKU 与数据并总结） | 是 | 是 | 是 | `agent` 层 runner + `module/vision` |
| `submit_image_plan` | 提交一条确定性图片处理 DAG 提案进确认队列（校验 + 入队，不执行） | 否 | 是 | 否 | `module/dag` |
| `submit_imagegen_plan` | 提交一条生图提示词提案（源图引用 + 提示词）进确认队列（校验 + 入队，不执行） | 否 | 是 | 否 | `module/imagegen` |
...
- **入参**：`type`（必填，枚举 `vision` / `explore`）、`prompt`（交给子 Agent 的任务描述）、按 type 附带的上下文（如 vision 的图片引用 `imageIds`）。
...
- 本期两种 type **均为只读**：
  - `vision`：看图（Qwen-VL 类多模态），输出结构化视觉描述/评估。
  - `explore`：搜索关联 SKU 及其数据，汇总情报回主 Agent。
...
- **入参**：`dag`（必填，Agent 直接产出的 DAG JSON：nodes/edges，节点 `tool` 仅限像素工具白名单）、可选 `note`（给用户的方案说明）。
- **行为**：调 `module/dag` 的 `DagValidator` 做服务端独立严格校验（结构 → 节点数 → 白名单 → 参数 schema → 边引用 → 无环）；通过则把该 DAG 作为**待确认提案**入队（pending plan 记录，归属 `module/dag` / `module/task`），返回提案引用（plan id + 摘要）供 Agent 告知用户。
- **不执行**：实际像素处理由用户在带外确认后触发（确认 → 令牌 → 执行，见 [八.3](#83-hitl-在带外工具层零令牌)）。
- 多变体并行：白底一条、蓝底一条 = 两次工具调用（可并发）；有序步骤（蓝底→压缩）放同一条 `dag` 的 edges。
- 非只读、可并发、Plan 模式**不可见**。

### 3.5 submit_imagegen_plan

- **入参**：`source_image_ids`（必填，源图引用）、`prompt`（必填，Agent 综合数据/记忆/用户要求撰写的生图提示词）、可选 `note`。
- **行为**：校验源图引用与提示词形状后，把「生图提示词提案」入确认队列，返回提案引用。**不直接调生图模型**。用户确认后带外执行（通义万相类），产物落 MinIO + `process_result`。
...
**不依赖项**：
- **不依赖 `contracts`**：工具集零令牌，不碰 `ConfirmationToken` / `TokenClaims`。
...
public record PermissionInputs(
    boolean readOnly,
    /* confirmationAction 恒为 null：工具集零令牌 */
    Map<String, Object> subjectMetadata
) {}
```

- 分类**基于本次输入**：`agent` 的 `type` 决定 readOnly（vision/explore 只读）；`read` 恒只读；`submit_*_plan` 恒非只读。
- **fail-closed 默认**：`classifier` 抛异常或缺失 → 按 `readOnly=false`、`concurrencySafe=false` 处理，且权限按需鉴权。
- **本期所有工具 `confirmationAction=null`**：零令牌（见 [八.3](#83-hitl-在带外工具层零令牌)）。该字段保留是为兼容 `PermissionSubject` 现有签名，不代表工具层有令牌闸门。
...
```
[search, read, agent(vision), submit_image_plan(白底), submit_image_plan(蓝底)]
  → 批1: search, read, agent(vision)        并发（只读）
  → 批2: submit_image_plan×2                 并发（提案互相独立、入队幂等）
```

> `submit_image_plan` / `submit_imagegen_plan` 标 `concurrencySafe=true`：每条提案是独立的「校验 + 入队」，无共享可变状态冲突；入队应保证幂等（同 `toolCallId` 重复不产生重复 pending plan）。
...
执行器在 permission 步把分类翻译成已落地的 `PermissionSubject`（见 `pixflow-permission`）：`toolName`、`readOnly`、`confirmationAction`（本期恒 `null`）、`conversationId`、`packageId`、`payloadHash`、`actualCount`、`metadata`，交 `PermissionPolicy.evaluate(subject, context)`。`PermissionContext` 由 loop 提供（含 `conversationId`、`subagent` 约束、`deniedTools`/`disabledTools`、Plan 模式注入的 denied 集合）。

> `PermissionSubject` 现有签名要求 `packageId`/`payloadHash` 非空。对 `search`/`read`/`plan` 这类无包上下文的工具，executor 以约定占位值（如当前会话绑定的 packageId 或哨兵值）填充；这点需与 `permission` 对齐——**可能需要把这两个字段放宽为可空**，作为本文向 permission 模块提出的接口微调建议（见 [十八](#十八暂不考虑) 之外的「待对齐项」）。
...
这是本模块与 `design.md` 原 `submit_dag(需令牌)` 的关键差异：

- Agent 调 `submit_image_plan` / `submit_imagegen_plan` 只**产生待确认提案**（校验 + 入队），无真实副作用，**不需要也不携带确认令牌**。
- 真正的执行（像素处理 / 生图）由用户在前端逐条确认后，经 `module/conversation` 的**确认 REST 端点**触发：该端点经 `permission` 的 `ConfirmationTokenService` 签发/校验令牌，再驱动 `module/task` / `module/imagegen` 执行。
- 因此**确认令牌闸门整体在工具层之外**。Agent 没有任何能扣下真实副作用扳机的工具——这把「安全边界是硬约束」落到了工具空间本身（连扳机工具都不存在），比令牌闸门更强。
- `contracts` 的确认令牌契约只服务于确认 REST 端点 + permission + infra/cache，**与 `harness/tools` 无关**。
...
- 允许：`search`、`read`、`agent(vision/explore)`、`plan_exit`。
...
- **分类 input-aware**：`agent(vision)` 只读、`submit_image_plan` 非只读；分类异常 fail-closed。
- **permission 短路**：DENY 不调 handler、归一化为 `PERMISSION`；`confirmationAction` 恒 null（断言工具集零令牌）。
...
- **工具层确认令牌**：HITL 在带外确认 REST 边界，工具集零令牌；令牌契约不进本模块。
