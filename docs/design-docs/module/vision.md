...
├── enrich/
│   ├── CopyEnrichmentTopology.java    # 用 QueueTopologyBuilder 声明 pixflow.vision 队列组
...
**目标**：消费 file 在解压终态投递的 `CopyEnrichmentMessage`，按 SKU gap-fill 写 `asset_copy`，doc 文案优先、VLLM 仅补缺口；端到端在 Testcontainers MinIO + RabbitMQ + MySQL 上跑通。

**范围**：

- `CopyEnrichmentTopology`（`pixflow.vision` 队列组）。
...
| `CopyEnrichmentTopology` | B | 始终装配 | `pixflow.vision` 队列组 |
...
- 阶段 B：集成测试用 `Testcontainers` 启动 MinIO + RabbitMQ + MySQL + fake `VisionModelClient` + fake `infra/image`；按 `infra/storage` 既有 profile 跳过策略处理 Docker 不可用。
...
- **失败隔离**：单 SKU 抽取失败 → 记录跳过、其余 SKU 继续；整包可 ack（部分成功）。VLLM 持续不可用/毒消息 → 经 `ConsumerErrorHandler` 重试耗尽进 `pixflow.vision.dlq`。
...
- **两路调用方各自处理失败**：
  ...
  - 富化面：失败交 `CopyEnrichmentErrorHandler`（实现 `infra/mq.ConsumerErrorHandler`）按 `recovery` 给 `RetryDecision`（抖动 `Retry`→耗尽 `DeadLetter`；单 SKU `SKIP` 类不冒泡，整包仍 ack 为部分成功）。
...
pixflow:
  vision:
    ...
    enrich:
      ...
      consumer-timeout: 30m          # pixflow.vision.q 的 ack 超时（部署侧同步 RabbitMQ）
```

- 模型型号不在此配置：VLLM 型号由 `infra/ai` 的 `pixflow.ai.roles.vision` 承载（design.md「型号配置化」）。
- 队列/交换机/路由键由 `CopyEnrichmentTopology` 用 `QueueTopologyBuilder("pixflow.vision")` 声明，不在配置硬编码业务队列名（`infra/mq.md §四/§十`）。
...
| `infra/mq` | 仅富化面：`CopyEnrichmentTopology` 声明 `pixflow.vision` 队列组；`CopyEnrichmentConsumer` process-then-ack；`CopyEnrichmentErrorHandler` 实现 `ConsumerErrorHandler`；新增 `mq → vision` 依赖边 |
...
- **富化作业集成测试（Testcontainers：RabbitMQ + MinIO + MySQL）**：发富化消息 → 消费 → 按 SKU 抽取 → gap-fill 写 `asset_copy`；doc 已有文案的 SKU 不被覆盖；重投幂等（已填充 SKU 跳过）。
