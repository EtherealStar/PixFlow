...
### 4.4 `PermissionPolicy` —— 评估入口

```java
public interface PermissionPolicy {
    PermissionDecision evaluate(ToolDescriptor descriptor,
...
- `evaluate()`：每次工具调用执行前，由 `harness/tools` 执行管线在 `classify → guard → permission` 步骤调用。
...
sequenceDiagram
    ...
    Tools->>Perm: evaluate(submit_dag, classification, context)
...
`submit_dag`/imagegen 执行前，`ConfirmationTokenService.verifyAndConsume` 依次校验：
...
```mermaid
flowchart TD
  Start["evaluate(descriptor, classification, context)"] --> A1{"子 Agent 约束违例?<br/>(read-only 调副作用/非白名单工具)"}
...
1. `context.isSubagent()` 且 `subagent.readOnly` 且当前 classification **非只读**（`classification.read_only == false`） → `DENY`。
...
| 对接方 | 契约 |
|---|---|
| `harness/tools` 执行管线 | 在 `validate → classify → guard → **permission** → PreToolUse hook` 步骤调 `evaluate`；`DENY`/`CONFIRM_REQUIRED` 在 handler 前短路返回结构化结果（参考 tool-runtime 的 `_prepare_input`） |
