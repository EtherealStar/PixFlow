...
- 已按设计避免 task 直连 `module/file`，通过 `api.port.TaskAssetReader` 暴露资产读取 SPI。真实 E2E 需要在 file/app 侧补一个实现，把 `asset_image` 映射为 dag `ImageDescriptor` 与生成式 `GenerativeSource`。
...
- 留待后续迭代：补 `TaskAssetReader` 的真实 file/app 实现；补真实数据库/Redis/RabbitMQ/MinIO 端到端 IT；把 `DownloadBundleBuilder` 从内存 zip 改为真正适合 2GB 上限的流式上传；如 state 模块后续接受 `GENERATIVE`，再扩展 checkpoint 投影；按启动期错误码聚合机制纳入 `TaskErrorCode` i18n/唯一性检查。
