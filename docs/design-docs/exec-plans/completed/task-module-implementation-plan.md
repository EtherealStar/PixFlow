...
- 2026-06-29T23:08:00+08:00：实现 `TaskStateMachine` / `ResultStateMachine`、`TaskErrorCode`、`IdempotencyGuard`、Redis cache key/counter/cancel/idempotency adapter、task lock adapter、Micrometer `TaskMetrics`。
- 2026-06-29T23:09:00+08:00：实现 `CreateTaskServiceImpl`、`CancellationService`、`TaskQueryServiceImpl`、`DownloadService`、`DownloadBundleBuilder`、`TaskEventPublisher`、`ProgressAggregator`、`FailureIsolator`、`TerminalStateJudge`、`RecoveryService`、`HeartbeatWriter`。
- 2026-06-29T23:10:00+08:00：实现 `TaskMessagePublisher` / `TaskMessageListener` / `TaskMessage`、`WorkUnitScheduler`、`WorkerRouter`、`TaskWorker`、`ProcessWorker`、`ImageGenWorker`。确定性路径只调用 `BranchExpander` + `UnitExecutor`；生成式路径只调用 `ImageGenExecutor`。
- 2026-06-29T23:11:00+08:00：实现 `TaskAutoConfiguration` 与 Spring Boot auto-configuration imports；task 模块在存在 `ImageGenClient`、`ObjectStorage`、`ImagegenProperties` 且未自定义 `ImageGenExecutor` 时主动装配 `DefaultImageGenExecutor`。
- 2026-06-29T23:12:00+08:00：新增状态机单测与 task 架构边界测试；执行 `mvn -pl pixflow-module-task -am test` 通过；执行 `mvn -pl pixflow-app -am test` 通过；最后迁移 `TaskAssetReader` 到 `api.port` 后执行 `mvn -pl pixflow-module-task -am test -DskipTests` 与 `mvn -pl pixflow-app -am test -DskipTests` 通过。
- 2026-07-16T16:59:00+08:00：补齐 `TaskAssetReader` 的 app 级真实适配器、生成式源图冻结快照与派生重试子集重建；创建路径统一接入 `PendingTaskEnqueuer`，MQ 发布失败恢复 PENDING；接通 `ProgressEvent → ConversationProgressBridge → STOMP`；批量下载改为受上限保护的临时文件流式 ZIP；补齐 heartbeat/recovery/pool 指标。Task 模块共 30 项测试（2 项因本机 Docker 不可用跳过，28 项执行通过），完整 app reactor 干净编译通过，新增跨模块定向测试通过。

## Surprises & Discoveries

<!-- 实现过程中发现的意料之外的点（库行为、依赖冲突、API 不匹配等），每条带证据片段。 -->

- 2026-06-29T23:05:00+08:00：执行计划使用 `pixflow-harness-state` 表述，但仓库实际 Maven 模块名是 `pixflow-state`，包名仍为 `com.pixflow.harness.state.*`。实现按真实模块名接入。
- 2026-06-29T23:06:00+08:00：`harness/state` 当前 `UnitKind` 只有 `BRANCH` / `GROUP`，没有 `GENERATIVE`。task 内部保留 `domain.model.UnitKind.GENERATIVE` 用于生成式结果；`CheckpointReadPortImpl.loadCompletedUnits` 只向 state 暴露 state 当前可表达的 BRANCH/GROUP 成功单元。
- 2026-06-29T23:07:00+08:00：`infra/storage.ObjectStorage` 真实方法名是 `presignGet(ObjectLocation, Duration)`，不是计划文本中的 `presignedGetUrl`；实现使用真实接口。
- 2026-06-29T23:08:00+08:00：`TaskQueryService` 计划中的 `org.springframework.data.domain.Page` 不在 task 当前依赖图中自然可用。为保持 API 轻量，实际使用 task 自有 `PageResult<T>` record。
- 2026-06-29T23:09:00+08:00：`ResultSelector.bundle()` 静态工厂会与 record 组件 accessor `bundle()` 冲突；实际命名为 `ResultSelector.allResultsBundle()`。
- 2026-06-29T23:10:00+08:00：`process_result` 设计没有独立 `member_id` 列，成员标识分布在 `image_id` / `group_key`。`ProcessResultMapper.findByUnit` 按 `(task_id, branch_id, image_id|group_key)` 查询。
- 2026-06-29T23:11:00+08:00：`imagegen` 模块默认不暴露 `DefaultImageGenExecutor`。task auto-configuration 增加默认 `ImageGenExecutor` 装配，保持"imagegen 不自建异步，task 主动消费执行器"的边界。
- 2026-07-16T16:40:00+08:00：仅提供 `TaskAssetReader` SPI 仍会让生产启动缺 bean；同时生成式 worker 每次执行重新读取 `asset_image`，会绕过创建时冻结的输入事实。证据：`TaskAutoConfiguration` 强依赖该 SPI，而工作区无实现；`ImageGenWorker.plan` 直接调用 `sourceImage`。现由 app 组合层实现 file→task 投影，并把 source location/sku/id 写入 `unit_selection_json`，恢复只读冻结快照。
- 2026-07-16T16:45:00+08:00：已有 `ConversationProgressBridge` 只有测试调用点，Task 的 Spring `ProgressEvent` 没有生产 listener，导致前端订阅不到帧。现由 app 级 `TaskProgressEventBridge` 查询会话归属并转发到既有 `task-progress-{conversationId}-{taskId}` topic。

## Decision Log

<!-- 记录执行期间所有重要决策、理由、时间 / 作者。 -->

- 2026-06-29T23:05:00+08:00 / Codex：保留 `TaskAssetReader` 作为 task 对资产读取的 SPI，并放入 `api.port`。理由：task 不能依赖 `module/file`，但 file/app 侧后续需要一个稳定扩展点按 `packageId` 解析 `asset_image`。
- 2026-06-29T23:06:00+08:00 / Codex：生成式单元内部使用 task 自有 `UnitKind.GENERATIVE`，不修改 `pixflow-state` 的 `UnitKind`。理由：保持本次变更范围在 task 模块内，避免扩大 state 模块契约；对 state checkpoint 只投影其当前可表示的 BRANCH/GROUP。
- 2026-06-29T23:07:00+08:00 / Codex：`TaskQueryService.listByConversation` 返回 `PageResult<TaskSummary>`，不引入 Spring Data `Page` 到 API。理由：减少 API 对 Spring Data 的耦合，并解决当前依赖图编译缺口。
- 2026-06-29T23:08:00+08:00 / Codex：`DownloadBundleBuilder` 当前采用流式读对象、内存 `ZipOutputStream` 后写 TMP 桶，并用 `max-bundle-bytes` 做硬上限。理由：完成可用 bundle 分发能力；后续若要支持接近 2GB 的真实大包，应替换为对象存储分片/临时文件流式上传实现。
- 2026-06-29T23:09:00+08:00 / Codex：task auto-configuration 主动提供默认 `DefaultImageGenExecutor` bean。理由：imagegen 设计中默认隐藏 executor，Wave 4 task 是第一个真实消费者，应由 task 显式打开消费边界。
- 2026-07-16T16:42:00+08:00 / Codex：`TaskAssetReader` 的真实实现放在 `pixflow-app`，不放入 file 或 task。理由：app 同时依赖两个模块，是组合根；这样 file 继续不反向依赖 Wave 4 task，task 的 ArchUnit 边界也保持不变。
- 2026-07-16T16:47:00+08:00 / Codex：bundle 使用受 `max-bundle-bytes` 限制的临时文件生成 ZIP，再以已知长度流式上传 TMP 桶。理由：当前 `ObjectStorage.put` 需要预先提供长度，直接对象存储流式写不可用；受限磁盘文件避免 2GB 归档进入 JVM 堆，并能在 finally 中确定清理。

## Outcomes & Retrospective

<!-- 完成度、关键交付清单、设计 vs 实现一致性、实施期发现、留待下个迭代等。 -->

- 已交付一个可编译、可测试、可由 Spring Boot 自动装配的 `pixflow-module-task` 模块，覆盖 API、三张表数据模型、状态机、幂等创建、取消、MQ、worker fan-out、确定性/生成式路由、失败隔离、终态判定、恢复扫描、查询、下载、state checkpoint adapter、metrics 与架构守护。
- 已按设计保持 task 是 `dag` / `imagegen` 的异步外壳：`ProcessWorker` 只调用 `BranchExpander` / `UnitExecutor`，`ImageGenWorker` 只调用 `ImageGenExecutor`，task 内不引入像素工具语义或模型调用细节。
- 已按设计避免 task 直连 `module/file`，并在 app 组合根通过 `FileTaskAssetReader` 把可见 `asset_image` 稳定映射为 dag `ImageDescriptor` 与生成式 `GenerativeSource`；生成式输入也进入冻结 selection，worker 恢复不再重读可变素材行。
- 已实现 `TaskAutoConfiguration` 并接入 app 依赖，验证 `pixflow-app` 当前 reactor 编译与测试不因 task 模块接入而破坏。
- 已完成验证：`mvn -pl pixflow-module-task -am test` 通过，`mvn -pl pixflow-app -am test` 通过；迁移 `TaskAssetReader` 后的 `mvn -pl pixflow-module-task -am test -DskipTests` 与 `mvn -pl pixflow-app -am test -DskipTests` 通过。
- 2026-07-16 收尾后，原列出的代码缺口已关闭：真实素材适配已接通；bundle 不再内存聚合；state checkpoint 已覆盖 GENERATIVE；execution-domain 计划已补 MySQL/Redis/MinIO fencing/孤儿对象测试。当前机器没有可用 Docker，因此本轮未重新执行容器测试（测试代码保留，非容器 Task 测试 28 项全部通过）。
...
- 2026-06-29 / Codex: 同步实现结果。`pixflow-module-task` 已新增并接入根 POM 与 app，完成 API、持久化、状态机、worker 外壳、MQ/cache/storage/state/imagegen/dag 接线、下载、恢复、metrics 与测试。记录实际实现与计划差异：state 暂无 `GENERATIVE`、storage 方法名为 `presignGet`、查询分页采用 `PageResult`、`TaskAssetReader` 作为 `api.port` SPI、bundle 当前为受 `max-bundle-bytes` 保护的内存 zip 后写 TMP 桶。
- 2026-07-16 / Codex: 完成历史遗留收尾。app 组合根新增真实 `TaskAssetReader` 和 Task 进度 STOMP bridge；生成式任务在创建时冻结源图对象位置并支持 derived retry 子集；普通/生成式 worker 都以 selection 为执行事实；创建入队统一使用可补偿 `PendingTaskEnqueuer`；bundle 改为受限临时文件流式上传；补齐关键指标与六组回归测试。原因是原 Outcomes 明确列出的 adapter、内存 ZIP 和真实推送仍是生产断点。
