...
- [x] (2026-07-19) Milestone 1 验收完成：Storage candidate key 与 Pixel Budget 各 4 项测试通过；本计划直接涉及的 AI、File、DAG、Imagegen、Task 模块测试在串行定向 reactor 中均通过，零跳过。
...
- [x] (2026-07-19) Milestone 2 验收完成：真实 MySQL 8.4 的 `ExecutionFencingIntegrationTest` 2 项验证旧 owner affected-row 为零、higher owner 保留 immutable result epoch 并只绑定一次；真实 Redis 的 ownership failure injection 1 项通过；publication backlog/failure/replay 定向测试证明失败不改写 provider/DAG 结果。
...
- [x] (2026-07-18 23:31+08:00) Milestone 3 验收完成：`GeneratedImagePublicationIntegrationTest` 使用真实 MySQL 8.4 + MinIO，以 5 项零跳过测试覆盖并发 publisher 唯一性、有序单源/多源 lineage、reservation/copy/READY/cleanup crash windows 及 candidate/stable 双缺失诊断。
...
- [x] (2026-07-19) Milestone 4 验收完成：Task 模块完整测试已覆盖 auto-configuration/coordinator，App publication/source/custom-download boundary 4 项通过；生产搜索确认 Task→File、File→Task/Imagegen、App→File persistence/internal 和 App custom download→Task non-api 均无命中。
...
- [x] (2026-07-19) Milestone 5 验收完成：Task 删除已发布结果只 soft-delete 执行投影并保留 `generatedImageId/referenceKey`；File 删除 Generated Image 后历史 Task 仍返回执行身份但不再生成失效 preview；App source adapter 证明 READY Generated Image 可再次作为 Imagegen source，公开查询仍只暴露 PUBLISHED identity。
- [x] (2026-07-19) Milestone 5 的 public visibility 与 source reuse 定向验收完成：Task outcome/query/custom-download 只返回 PUBLISHED identity 的定向测试、App typed source/publication 的 4 项测试及 Imagegen canonical IMAGE 校验的 5 项测试均通过；Rubrics 旧 `ObjectLocation` fixture 已切到 `generatedImageId/referenceKey`。custom download query 显式拒绝 SUCCESS/PENDING，即使异常数据已有 referenceKey 也 fail closed。
...
- [x] (2026-07-19) Milestone 6 验收完成：File 并发重放与真实 MySQL/MinIO crash-window、Task MySQL owner fencing、Redis disconnect/ownership fault、orphan candidate、删除独立性与 Derived Retry 全部通过；Derived Retry 改为后端固定 `retry-failed:{sourceTaskId}`，前端不再发送随机 Idempotency-Key。
...
- [x] (2026-07-19) Milestone 7 验收完成：30 模块 `-DskipTests verify` 在 8:28 内 BUILD SUCCESS；Task 43、File 50、Imagegen 56、App 23、Cache 20 项测试均零失败/错误/跳过；前端 lint、typecheck、115 tests 和 build 通过；父计划、专项计划和用户可观察生命周期合同已同步，专项计划移入 completed。
...
- [x] (2026-07-19) 增量收尾验证：Docker 29.5.3 下 File publication 5 项、Task MySQL fencing 2 项、Task orphan object 1 项、Redis ownership fault 1 项均零失败/零跳过；File、DAG、Imagegen、Task 完整模块测试及 App 边界 4 项通过。用户明确要求不运行全仓测试，整体计划仍因生命周期端到端验收未完成而保持活动。

## Surprises & Discoveries

- Observation: Task 的 orphan-object fresh-schema 集成测试仍只加载 V1，生产查询新增 V2 publication 列后会在真实 MySQL 报 `Unknown column 'r.publication_status'`。
  Evidence: 将 `OrphanedResultObjectIntegrationTest` 初始化对齐 V1+V2 后，Task 完整 43 项测试及真实 MySQL/MinIO orphan 场景零失败、零跳过。

- Observation: Task 列表曾用 `PublishedAssetReader.require` 生成 preview；用户从 File 删除 Generated Image 后，历史执行结果查询会因资产缺失整页失败。
  Evidence: `PublishedAssetReader` 增加 `find` 后，`DownloadService.previewResult` 对已删除资产返回空 preview，而显式 download 仍用 `require` fail closed；`DownloadServiceTest` 与 `TaskQueryContractTest` 固化该行为。
...
- Observation: 上述两个跨包债务可以在不扩大业务接口的情况下消除。
  Evidence: `PublicationCoordinator` 已移动到 worker package 并收窄为 package-private；Task 新增 owner-defined `api.download` bundle/result query，App custom download 生产源码只 import Task API，sentinel test 与模块静态门禁均通过。
...
- Decision: Derived Retry 的业务幂等身份固定为 `retry-failed:{sourceTaskId}`，HTTP 客户端不再提供请求键。
  Rationale: terminal source 与失败 Work Unit 快照不可变；让客户端随机键参与身份会允许同一来源创建多个直接派生 Task，违反 ADR 0003。
  Date/Author: 2026-07-19 / Codex

- Decision: Published Asset 读取同时提供可选 `find` 和强制 `require`；历史 Task 列表使用 `find`，显式下载、Rubrics 和 bundle 使用 `require`。
  Rationale: File 删除资产后历史执行事实仍应可诊断，但显式消费缺失字节必须失败，不能把删除误报成可下载。
  Date/Author: 2026-07-19 / Codex
...
截至 2026-07-19，本计划已完成 candidate → checkpoint → File reservation/copy/READY → Task bind 的完整闭环。实际新增 Task V2 和 File V4 migration；Task/File 通过 owner-defined public API 由 App adapter 组合，Task↛File、File↛Task/Imagegen、App↛File persistence/internal，custom download 也不再依赖 Task internal/persistence。Task suppression 从 57 条清到零。
...
真实 MySQL/Redis/MinIO 的 publication crash-window、并发 publisher、owner fencing、orphan object、Redis 断连/ownership fault 已完成且零跳过。30 模块严格静态 reactor 在 8:28 内 BUILD SUCCESS；Task 43、File 50、Imagegen 56、App 23、Cache 20 项测试全部通过；前端 lint、typecheck、115 tests 和 build 通过。删除 Task 结果不删除 Generated Image，删除 Generated Image 不破坏历史 Task 诊断，READY Generated Image 可再次作为 source；Derived Retry 来源不变且只产生一个固定业务身份的直接子 Task。本计划目标全部完成并已移入 `exec-plans/completed/`。
...
2026-07-18 / Codex: Docker 恢复后完成 File publication 的真实 MySQL 8.4 + MinIO 验收。`GeneratedImagePublicationIntegrationTest` 5 项零失败、零跳过，覆盖同一 result 并发唯一 READY identity、有序 lineage、reservation 后恢复、copy 后 READY 前恢复、READY 后 cleanup 恢复和双对象缺失诊断；Milestone 3 现已完成。Task/Redis fencing、删除独立性、Derived Retry 和产品生命周期端到端仍属于 Milestone 6/7，专项计划保持活动状态。

2026-07-19 / Codex: 完成定向收尾并移除两项架构债务。`PublicationCoordinator` 收窄到 worker package-private，custom download 改经 Task-owned `api.download`；真实 MySQL fencing 2 项、MinIO/MySQL orphan object 1 项、Redis ownership fault 1 项、Storage candidate key 4 项、Pixel Budget 4 项及 App boundary 4 项均通过且零跳过。File/DAG/Imagegen/Task 完整模块测试在目标 reactor 中通过；全仓尝试先后遇到范围外 Eval 偶发失败和 Conversation 测试夹具缺 Bean，用户随后明确要求不再全仓测试。计划因删除独立性和 Generated Image source reuse 的端到端验收仍未完成而保持活动。

2026-07-19 / Codex: 完成 Milestone 3 最终收尾。Derived Retry 改为后端固定业务幂等身份并同步前端无 header 合同；修复 orphan fresh-schema 漏载 V2；为 Published Asset 增加可选读取，使 File 删除 Generated Image 后 Task 历史仍可诊断且显式下载继续 fail closed。30 模块严格静态 reactor、Task/File/Imagegen/App/Cache 完整模块测试、真实 MySQL/MinIO/Redis 故障测试及前端四门禁全部通过，Milestone 5/6/7 勾选完成并归档本专项计划。
