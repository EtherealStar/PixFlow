...
目标 controller 清单必须从实施时的 `frontend/api.md` 逐项抄录并核对。创建计划时确认的顶层分组是 Auth、Conversations/Messages/Proposals、Asset References、Uploads/Packages/Images、Product Visual Facts、Global Activity、Tasks/Outputs/Downloads；Rubrics、Settings、Register、legacy Files 和 standalone Task detail 不在合同中。
