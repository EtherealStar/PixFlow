...
- [x] (2026-07-20) Milestone 7 前端合同文档切片：按前端目标设计补全 `frontend/api.md` 的 wire DTO、Materials 图片详情、Outputs、Product Visual Analysis、Activity snapshot/frame/commands 和 removed routes；Cancel/Retry failed/Clear 统一使用 Activity Command。只完成目标规范冻结，未修改 `pixflow-web` 或当前 Controller，Milestone 7 保持未勾选。

## Surprises & Discoveries

- Observation: 前端权威 API 摘要在 7 月 20 日 App 重构前已描述 Outputs/Activity 目标，但字段不足以直接实现前端，并残留 direct Task retry，与 Activity-only command surface 冲突。
  Evidence: 旧 `frontend/api.md` 没有 Materials image-detail、Outputs/Activity 完整 DTO，却同时声明 standalone Task API removed 和 `POST /api/tasks/{taskId}/retry-failed`；前端 `tasks.md` 与 App 专项计划要求命令由 Activity runtime/route 统一承接。
...
- Decision: `frontend/api.md` 只描述目标 wire contract，不记录当前过渡实现；前端的 Cancel、Retry failed items、Clear 全部以 opaque `activityId` 调用 Activity Command，App 路由到 File/Task owner，direct `/api/tasks/**` 不属于前端公开面。
  Rationale: 目标文档必须能独立指导后续 Web 重构，不能因当前旧 Controller 尚存而恢复双合同。Activity 统一交互面与 owner-owned 业务规则可以同时成立。
...
前端目标合同现已可作为后续重构输入：它明确区分 Materials、Outputs、Activity、Proposal 和 Product Visual Analysis，固定安全 wire DTO 与 removed routes，并消除了 direct Task command 与 Activity command 双入口。当前后端/前端实现差异仍是 Milestone 7 的实施工作，本次文档收敛不代表该里程碑完成。
...
2026-07-20 / Codex: 按前端目标设计同步接口参考文档。补齐 common envelope、公开分页/DTO、Conversation history 与安全 SSE、Asset Reference picker、chunk upload、Materials image detail、Product Visual Analysis、Outputs、Activity snapshot/frame/commands 和 canonical bundle download；把 Cancel/Retry failed/Clear 统一到 Activity Command，并明确 direct Task routes 等 removed contracts。同步更新 Web glossary、前端 Activity 设计和 App 专项 living sections；未修改生产源码、前端实现或里程碑完成状态。
