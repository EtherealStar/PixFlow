...
- [ ] 新增 `pixflow-session` Maven 模块并接入根 `pom.xml` 与 dependencyManagement。
- [ ] 在 `pixflow-infra-storage` 中抽取共享 `toolresult` 包，并让 context 后续可迁移到该共享抽象。
- [ ] 用 migration 风格新增 `message` 与 `message_compaction` schema。
- [ ] 实现 `TranscriptPort` 的 append/load/replaceForCompaction 全链路。
- [ ] 补齐单元测试、架构测试、migration/schema 测试和可选 Testcontainers 集成测试。
...
当前计划完成的是文档同步和实施规格，不包含 Java 代码。已经明确的关键结果是：session 不是 MVP 版消息 CRUD，而是 MySQL append-only 事实源、压缩纪元解释器、共享 tool-result 外置消费者和 `message` 表唯一写者。执行本计划时，任何把 Redis、prompt 组装、语义召回、conversation 写消息或 eval trace 写入塞进 session 的做法都应视为越界。
