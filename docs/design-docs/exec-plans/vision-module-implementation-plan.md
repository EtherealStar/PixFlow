...
- [ ] (里程碑 1 进行中) 阶段 A 里程碑 1：`pixflow-module-vision` Maven 模块骨架，加入根 `pom.xml` 与 `dependencyManagement`，源码包 `com.pixflow.module.vision`，`mvn -pl pixflow-module-vision -am compile` 不依赖 Spring 上下文。
- [ ] (里程碑 2 待办) 阶段 A 里程碑 2：实现 `VisionImageRef` / `VisionTaskType` / `VisionAnalysisRequest` / `VisionAssessment` / `VisionAnalysisResult` record + `VisionAnalysisRequestValidator` 浅层校验；8 个单测覆盖枚举/可空/字段语义。
- [ ] (里程碑 3 待办) 阶段 A 里程碑 3：实现 `VisionImageResolver`（`infra/storage.getStream`）+ `VisionImagePreprocessor`（走 `infra/image.ImagePipeline`，仅允许 `ResizeOp(max-long-edge=1280, mode=FIT, allow-upscale=false)` + `ConvertFormatOp(target=jpeg, quality=0.85)` + `EncodeSpec(flattenBackground=WHITE)`）；单测覆盖正常/单图解码失败/超 `max-image-bytes`/含透明通道铺白底/`preprocess_decodesOnce_encodesOnce` 计数桩验证 1+1 编解码。
- [ ] (里程碑 4 待办) 阶段 A 里程碑 4：实现 `VisionPromptBuilder`（system 强制 JSON 输出形状约束 + user 问题 + 图片引用元数据）+ `AssessmentParser`（剥离 markdown 代码块、字段缺失置空、解析失败 → `parseDegraded=true` + `rawText` + 低 `confidence` 但**不**抛异常）；10 个单测覆盖法包裹/字段缺失/非法 JSON/置信度低值。
- [ ] (里程碑 5 待办) 阶段 A 里程碑 5：实现 `VisionService.analyze(VisionAnalysisRequest)` 门面 + 采样策略 `MAIN_FIRST`（按 `viewId` 排序，主图优先，再补足到 `images-per-call`）+ 单图跳过策略（`issues` 标 `decode_failed` / `too_large` / `resolve_failed` 但其余图继续；全部失败才抛 `VISION_NO_DECODABLE_IMAGE`）；`VisionAnalysisResult` 含 `assessment` + `usage` + `imagesSent` + `parseDegraded`；fake `VisionModelClient` + fake `infra/storage` + fake `infra/image` 跑通端到端；infra/ai 抛 `MODEL_RATE_LIMITED`/`MODEL_CONTEXT_LIMIT`/`MODEL_NETWORK_ERROR`/`MODEL_PROVIDER_ERROR` 全量原样上抛，`errorPassthrough_throwsInfraAiExceptionUnchanged` 用 `assertSame` 锁死。
- [ ] (里程碑 6 待办) 阶段 A 里程碑 6：`VisionErrorCode` 4 条并入 `common` 启动期聚合目录（`VISION_NO_DECODABLE_IMAGE` / `VISION_IMAGE_RESOLVE_FAILED` / `VISION_IMAGE_TOO_LARGE` / `VISION_EMPTY_REQUEST`），`VISION_*` 前缀守全局唯一；`VisionProperties` `@ConfigurationProperties("pixflow.vision")`（`image` / `analyze` / `enrich` 三个嵌套类）。
- [ ] (里程碑 7 待办) 阶段 A 里程碑 7：`VisionMetrics` 3 类 Micrometer 指标（`pixflow.vision.analyze{taskType, parse=ok|degraded}` / `pixflow.vision.images{stage=received|sent|skipped}` / `pixflow.vision.enrich{result=filled|skipped|failed}`，阶段 B 才填后者，但 class 现在就建出来便于 agent 维指标的对照一致）；`VisionServiceAutoConfiguration` 装配 `VisionService` / `VisionImageResolver` / `VisionImagePreprocessor` / `VisionPromptBuilder` / `AssessmentParser` / `VisionMetrics`，**不**装配 `VisionToolHandler` / `VisionToolDescriptor`（Wave 5 才出现）。
- [ ] (里程碑 8 待办) 阶段 A 里程碑 8：ArchUnit `VisionArchitectureTest` 落地 14+ 条边界守护（不可 import 的包清单、不可出现的模型/供应商字面量、`VisionService.analyze` 不得 `catch (PixFlowException) { throw new RuntimeException(e) }` 等签名模式）。
- [ ] (里程碑 9 待办) 阶段 A 里程碑 9：Sentinel 单测 `VisionServiceAutoConfigurationSentinelTest` 用 `ApplicationContextRunner` + 自 stub `infra/ai` / `infra/storage` / `infra/image` Bean，断言 `VisionService` bean 存在、`VisionToolHandler` bean 不存在；`mvn -pl pixflow-module-vision -am test` 全绿；`mvn -pl pixflow-app -am test` 不退化。
- [ ] (里程碑 10 待办) 阶段 B 里程碑 1：`CopyEnrichmentTopology`（`QueueTopologyBuilder("pixflow.vision")`，主交换机 `pixflow.vision` direct、路由键 `vision.copy_enrich`、DLX `pixflow.vision.dlx`、DLQ `pixflow.vision.dlq`、`pixflow.vision.retry.q` 走原生 TTL+DLX 延迟重试）；`CopyEnrichmentErrorHandler` 实现 `infra/mq.ConsumerErrorHandler`（按 `RecoveryHint` 给 `Retry` / `DeadLetter` / `AckDrop`）。
- [ ] (里程碑 11 待办) 阶段 B 里程碑 2：`CopyEnrichmentConsumer`（`@Component` + `@RabbitListener`，`process-then-ack` 模式）→ 查 `asset_image` 按 `sku_id` 聚合 → 仅对「无 doc 文案或文案字段有缺」的 SKU 调 `ProductCopyExtractor.extractProductCopy(imageRefs, ctx)`（复用 `VisionService.analyze`，prompt 切到「SELLING_POINTS」任务，让模型给结构化字段）→ `AssetCopyWriteMapper` 走 `INSERT ... ON DUPLICATE KEY UPDATE` 按 `(package_id, sku_id)` 幂等 upsert。
- [ ] (里程碑 12 待办) 阶段 B 里程碑 3：`VisionEnrichErrorCode` 2 条（`VISION_COPY_EXTRACTION_FAILED` / `VISION_FILL_POLICY_REJECTED`）并入 common 启动期聚合；`fill-policy` 配置（`GAP_ONLY` 默认 / `SKIP_IF_ANY`）落地；`enrich.consumer-concurrency=2` / `prefetch=1` / `intra-package-parallelism=4`；Sentinel 单测断言 `CopyEnrichmentConsumer` 默认装配。
