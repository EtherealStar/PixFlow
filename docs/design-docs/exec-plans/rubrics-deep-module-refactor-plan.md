...
- [x] (2026-07-20 19:20+08:00) Milestone 1：严格模板、四态 Criterion、Evidence Pack identity、rule/LLM verifier、重复 rollout、majority、evidence citation 校验与 self-judged provenance 已完成；干净静态门禁零 Checkstyle/SpotBugs，最终非容器 Rubrics 回归 48 项全绿。
- [x] (2026-07-20 19:20+08:00) Milestone 2：旧 V1/V2 已由 fresh-schema 事实模型替换，`DefaultRubricsEvaluationService` 已直接接管 admission、claim epoch、lease/heartbeat、fenced transaction、resume、terminal checkpoint 与 bounded view；真实 MySQL schema/claim/service 集成测试已在 2026-07-19 零 skip 通过。
...
- [x] (2026-07-19 20:14+08:00) 完成后端 Dataset/Gold Label/校准/正式回归切片：不可变 manifest 注册按 canonical hash 幂等并拒绝 identity drift；Gold Label import 事务化，拒绝 `INCONCLUSIVE`，要求两个独立 annotator 且分歧必须由第三人裁定；`CALIBRATION` 只用完整 `HOLDOUT` 生成 criterion 级 kappa、macro-F1、agreement、错误率与 category/difficulty slices，精确绑定 dataset/template/evaluator/evidence schema；首个达标 formal run 可作为 baseline，后续 run 强制同 Dataset/version/snapshot 配对，`PASS -> FAIL` 生成幂等 Rubrics-owned alert。
- [x] (2026-07-19 20:14+08:00) 完成后端三类 Subject、durable automation 与观测切片：Task public seam 提供 immutable Copy 与 confirmed decision/DAG snapshot；event/scheduled admission 先持久化再入有界队列，稳定 admission key 去重，queue full 保留 PENDING，MySQL recovery 扫描 PENDING/RUNNING；每个 rollout 前 heartbeat；Micrometer 覆盖 run/item/criterion/judge/automation/queue/recovery，安全日志不记录 prompt、raw response、presigned URL 或 evidence 内容。
- [x] (2026-07-19 20:44+08:00) 完成双轴审查修复并编译通过：校准现在核对全部 `HOLDOUT item x criterion`，缺 evaluation 或任一双标注不会从 inner join 分母消失；文本 evidence 按 UTF-8 bytes 执行 64 KiB 上限；Task Decision 不再把 DAG payload 伪装成 Proposal；Subject Catalog 与 Evidence Factory 收敛运行引擎的平行 switch。用户随后要求“只更新文档”，因此这些最后修复只完成 Task snapshot install 与 Rubrics main/test compile，未继续运行测试或 lint。
- [x] (2026-07-19 22:35+08:00) 接通 Eval trace seam 并补 fail-fast/image 矩阵：rubrics pom 新增 `pixflow-eval` 依赖，新增 `TraceEvidenceProvider`/`DefaultTraceEvidenceProvider` 通过只读 `TraceQuery` 为 TASK_DECISION 投影有界 TRACE_SPAN 证据（按 turnNo 稳定排序、8 KiB span 截断、best-effort 缺失/读取失败返回空不伪造 span），`TextEvidencePackBuilder` 与 `RubricsAutoConfiguration` 接入（TraceQuery 为生产必需依赖，缺失 fail fast）。新增 `DefaultTraceEvidenceProviderTest`(7)、`RubricsAutoConfigurationTest`(2: judge 角色 fail fast + 禁止 `@ConditionalOnBean`)，并把 `ImageEvidencePackBuilderTest` 故障矩阵补到 4 项（缺失 NON_REPLAYABLE/损坏 INVALID_CONTENT/替换 INVALID_IDENTITY/正常 OUTPUT_IMAGE+IMAGE_METADATA 稳定 identity）。44 项单元测试零失败/错误/跳过，SpotBugs 零 finding，本次新增/修改文件零 Checkstyle violation。
- [x] (2026-07-20 19:16+08:00) 收紧 TASK_DECISION trace 归属：DAG/IMAGEGEN Proposal handler 在成功发布时把 owner `payloadHash` 写入受控 trace metadata；`DefaultTraceEvidenceProvider` 只接纳结构化 tool-call JSON 中该 hash 精确等于持久化 `decisionRevision` 的 turn，并从末两页精确截取最新 50 turn 的有界窗口。旧 Proposal、无法解析、非 Proposal 工具或不能证明归属的 trace 不进入 Evidence Pack。干净 Rubrics verify 与 App verify 均为零 Checkstyle/SpotBugs，48 项完整非容器 Rubrics 回归及 2 项 Proposal handler 测试通过；Docker daemon 未运行，真实 MySQL/MinIO 与完整 App production context 本轮仍无法复验。
- [x] (2026-07-20 20:23+08:00) 补强 Image immutable outcome：Task public snapshot 现包含持久化 producer provider/model，Rubrics 将其纳入 snapshot hash；Image evidence 读取后同时核验 published content size 与实际字节长度仍等于 Task 冻结的 `bytesOut`，同一发布 identity 的替换对象得到 `INVALID_IDENTITY`。定向 Image evidence suite 5 项零失败；依赖 reactor 超过桌面命令时限，但已生成的目标报告均为零错误。
- [ ] (2026-07-20 20:29+08:00) Docker MySQL/MinIO 均已健康启动；fresh schema 与 claim fencing 两份真实 MySQL Surefire 报告为零失败、零错误、零跳过。单模块 service integration 从本地仓库加载旧版 Task API，7 个测试报 `NoSuchMethodError`，不能作为验收；必须用完整依赖 reactor 复验。尚无真实 MinIO 的 image-evidence integration test，因此不以 MySQL 可用性替代 M3 对象 I/O 验收。
- [x] (2026-07-20 22:09+08:00) Milestone 3：完成 `IMAGE_RESULT` 纵向切片及真实对象证据验证。PublishedAssetReader 公开 File 冻结 contentHash，Rubrics 校验 imageId/size/bytes/hash；同长度替换、删除、损坏、像素预算拒绝和正常读取由真实 MinIO 5 项集成测试覆盖。
- [x] (2026-07-20 19:20+08:00) Milestone 4：不可变 Dataset manifest、双人 Gold Label 与第三人裁定、完整 HOLDOUT 校准、effective lifecycle、首个 formal baseline authority、同 Dataset/snapshot 成对回归和幂等 Rubrics-owned alert 已完成，并由真实 MySQL service 集成测试覆盖。
- [x] (2026-07-20 22:09+08:00) Milestone 5：完成 `COPY_RESULT` 与 `TASK_DECISION`，接入 Task public outcome 和 Eval read interface。Task public snapshot 提供 immutable Copy、confirmedProposal、DAG 和 revision；Rubrics 生成 COPY_TEXT/PROPOSAL/DAG_SNAPSHOT/TRACE_SPAN，真实 service integration 覆盖两类 Subject。
- [x] (2026-07-20 22:09+08:00) Milestone 6：完成 lifecycle-gated 离线自动化、组合根、观测与故障恢复。event/scheduled admission、bounded queue、recovery/watchdog、heartbeat、Micrometer 和 fail-fast App wiring 已由模块/App 架构与集成测试覆盖。
- [x] (2026-07-20 22:09+08:00) Milestone 7 后端范围：删除旧 Rubrics 生产代码、完成负向架构扫描、Rubrics suppression 清零并通过 lint-first 后端验收；前端产品表面因本次授权明确排除而不改。
- [x] (2026-07-20 22:09+08:00) 后端验收收口：TASK_DECISION trace 绑定 owner `decisionRevision` 并限制最新 50 turn；Task owner 显式提供 confirmedProposal；真实 MinIO/MySQL、Rubrics 69 项回归、Task outcome 2 项定向测试和 App 56 项测试均通过，Rubrics Checkstyle/SpotBugs suppression 已清零。
- [ ] Milestone 7 整体仍待完成：后端旧代码清理、负向架构扫描与 lint-first 验收已完成；用户明确要求“不做前端”，因此 `pixflow-web` 的 route/nav/store/api 删除不在本次授权内，不能将包含前端清理的整项标记完成。
...
- Observation: 本轮独立 Checkstyle 命令报告零 violation，但配置中仍有 61 条 Rubrics 精确 suppression；“零 violation”不能解释为 suppression 已清零。
  Evidence: `mvn -pl pixflow-module-rubrics -DskipTests checkstyle:check` 于 20:18 成功并报告 `0 Checkstyle violations`；最终负向搜索仍命中 `config/checkstyle/suppressions.xml` 61 行。用户要求 linting 时只记录 Checkstyle 问题、不修复，因此本轮不机械格式化这些旧文件，也不删除 suppression。

- Observation: 后续架构提交已清除旧 Rubrics suppression，但专项计划的 2026-07-19 进度文本仍保留“46/61 条”历史状态。
  Evidence: 2026-07-20 执行 `rg -n "pixflow-module-rubrics" config/checkstyle/suppressions.xml config/spotbugs/exclude-filter.xml` 无命中，干净 Rubrics verify 报告零 Checkstyle violation 与零 SpotBugs finding；旧数字只作为历史执行证据保留，不再代表当前工作树。

- Observation: 仅按 conversation 读取前 50 个含 tool call 的 turn 会把旧 Proposal trace 混入当前 Task Decision，且长会话的当前确认 turn 可能根本不在第一页。
  Evidence: 新增 `onlyUsesTraceForTheConfirmedDecisionRevision`、`ignoresPayloadHashOutsideSuccessfulProposalMetadata` 与 `readsTheLatestBoundedTraceWindowForLongConversations` 红灯测试，分别暴露跨 revision 泄漏、非受控字段伪造和首页窗口缺失。实现限定成功 Proposal 工具的受控 `result.payloadHash == decisionRevision`，再通过单行 total 查询和末两页合并精确截取最新 50 turn；`DefaultTraceEvidenceProviderTest` 最终 10 项通过。

- Observation: Proposal handler 原先只把 `payloadHash` 放在给模型看的 content，Tools/Eval trace 实际只持久化 `ToolHandlerOutput.metadata`，导致 Rubrics 的 revision 过滤在生产 trace 上永远无法命中。
  Evidence: `ProposalHandlersTest` 新增 metadata 断言后两项均红；DAG/IMAGEGEN 成功 handler 把同一 owner hash 写入受控 metadata 后两项通过。`RegistryToolExecutor` 与 `LoopToolTraceSink` 会把该 metadata 投影到 `TraceToolCall.result`，与 Rubrics 结构化查找的真实 JSON 形状一致。

- Observation: 分页查询的末页不一定包含完整“最新 50 turn”；例如 total=120 时末页只有 101-120，且先读取 50 条第一页取 total 会把总读取量扩大到 150 条。
  Evidence: 双轴审查指出边界后，测试用完整第 2 页和第 3 页证明 turn 70 必须排除、turn 71 必须保留；实现先用 `size=1` 取得 total，再合并末两页并截取尾部 50 条。证据窗口严格为 50，数据库返回行数上限固定为 101。

- Observation: 桌面执行器的 124 秒上限会在 Maven 已生成全部 Surefire 报告后终止外层命令，不能只用命令退出码判断测试是否执行完。
  Evidence: 完整 Rubrics test 命令超时后，16 份 Surefire XML 已全部生成，合计 39 tests、0 failures、0 errors、0 skipped；真实 MySQL 服务集成测试单独运行 10 项并正常 `BUILD SUCCESS`。最后双轴审查修复发生在这些测试之后，按用户“只更新文档”的最新指令未重跑。

- Observation: Task 当前持久 owner fact 只有 confirmed decision revision 与 Canonical DAG snapshot，没有独立 requirements/proposal，也没有 Eval trace identity。
  Evidence: 审查前 `TextEvidencePackBuilder` 曾把同一 DAG payload 同时标成 `PROPOSAL` 和 `DAG_SNAPSHOT`；修复后只发出真实 `DAG_SNAPSHOT`。trace-required criterion 仍应得到 missing evidence，不能合成 trace 或 Proposal。
...
- Decision: 用户后续要求“在 linting 时记录 checkstyle 问题但不用修复”；该要求覆盖本计划 Milestone 7 的本轮 suppression 清零动作。
  Rationale: Checkstyle 结果和剩余 suppression 数量必须作为证据保留，但不得为了 lint 结果扩大为机械格式化。本轮独立 Checkstyle 为零 violation，仍明确记录 61 条 suppression 未处理。
  Date/Author: 2026-07-19 / Codex

- Decision: confirmed Task Decision 只暴露 owner 真正持久化的 Canonical DAG snapshot；在 Task/Conversation/Eval 尚未提供 requirements、validated proposal 与 trace read identity 前，Rubrics 不创建同内容 Proposal，也不伪造 trace。
  Rationale: evidence type 是审计事实而不是展示标签。缺 owner seam 应产生 missing evidence 或保持里程碑未完成，不能用重复 payload 让 criterion 假通过。
  Date/Author: 2026-07-19 / Codex

- Decision: Calibration Report 必须覆盖全部 HOLDOUT subject 与模板全部 criterion；inner join 结果只用于取事实，不能定义分母。
  Rationale: 若缺 label、缺 evaluation 或 non-replayable item 从查询中消失，少量完整样本即可错误授予 automation/formal regression 资格。现在先计算 holdout manifest 数量，再逐 criterion 校验 subject 覆盖完整性。
  Date/Author: 2026-07-19 / Codex
...
- Decision: 本轮接通已存在的 Eval `TraceQuery` 只读 seam 作为 TASK_DECISION 的 TRACE_SPAN 证据来源，而非新增 trace 持久化。
  Rationale: `pixflow-eval` 已暴露 `TraceQuery`/`TraceReplay` 公开接口（按 `conversationId` 查询 turn），架构守护已允许 rubrics 依赖 `harness.eval.api`/`model`，禁止 `internal`/`persistence`。Rubrics 只读、不回写；trace 缺失/过期/读取失败按 best-effort 返回空，由声明 `TRACE_SPAN` 的 criterion 得到 `INCONCLUSIVE(MISSING_EVIDENCE)`，绝不伪造 span 或升级为 Subject 失败。criterion 可见性由既有 `EvidencePack.view(criterion.evidenceTypes())` 在类型层过滤（只有声明 `TRACE_SPAN` 的 criterion 才看到 trace 条目）；按 toolName 等的 criterion-level span 精选是后续 refinement，本轮不实现（judge 仍可从有界 span 集合中自行推理相关性）。`TraceQuery` 作为 `DefaultTraceEvidenceProvider` 的构造依赖，缺失即 fail fast。
  Date/Author: 2026-07-19 / Codex

- Decision: TASK_DECISION 的 TRACE_SPAN 必须在结构化 tool-call JSON 的受控 `result.payloadHash` 中精确绑定 Task owner 持久化的 `decisionRevision`；长会话只投影最新 50 turn，当前 TraceQuery seam 下最多读取 101 行，不能用整串文本包含或无界扫描兜底。
  Rationale: conversationId 只确定会话范围，不能证明某个 turn 属于当前确认决定。payload hash 是现有 Task/Conversation 边界共同持有的不可变 revision；无法解析或无法匹配时返回 missing evidence，比把旧 Proposal 当作证据更符合证据约束。单行 total 查询加末两页读取覆盖尾部窗口，并在没有倒序/offset API 时保持明确固定上限。
  Date/Author: 2026-07-20 / Codex

- Decision: Proposal 成功发布的 App adapter 必须把 owner `payloadHash` 同时写入 `ToolHandlerOutput.metadata`，作为 Eval trace 的最小稳定决定身份；错误结果不写该 metadata。
  Rationale: Tools trace 有意不持久化面向模型的完整 content，只记录受控 metadata。显式传递 hash 让 Rubrics 能通过真实 TraceQuery 数据绑定 Task decision revision，同时不记录 canonical payload、用户要求、完整 Proposal 或其他敏感内容。
  Date/Author: 2026-07-20 / Codex

- Decision: Task Decision 的 owner-defined requirements/proposal 本轮不实现，保持 MISSING_EVIDENCE。
  Rationale: 审计确认 Task 只持久化 `dag_json`（Canonical DAG）与 `payload_hash`，无独立 requirements/proposal；其来源是 conversation Proposal，而 conversation 不在本计划授权的 Task/Eval/App cross-module 范围内，且其工作树当前干净。按既有决定“缺 owner seam 应产生 missing evidence 或保持里程碑未完成，不能用重复 payload 让 criterion 假通过”，不新增 always-null nullable 列或同内容 Proposal 证据。`DAG_SNAPSHOT` 仍由 owner 真正持久化的 Canonical DAG 提供。
  Date/Author: 2026-07-19 / Codex

- Decision: 本轮验证受本机无 Docker 限制；模块级 `verify` 因 46 条 pre-existing Rubrics Checkstyle violation 失败，按“记录不修复”保留。
  Rationale: 本机 Docker daemon 不可用；既有集成测试用 `@Testcontainers(disabledWithoutDocker=false)`，无 Docker 时 error 而非 skip。本次新增/修改文件零 Checkstyle violation、SpotBugs 零 finding、44 项单元测试零失败。46 条 violation 位于 `RubricsJudgeRolloutMapper`/`EvaluatedCriterion`/`EvaluationSubject`/`ImageResultSubject`/`SemanticVersion`/`TemplateRegistry`/`TemplateValidator`/`RuleCriterionVerifier` 等 pre-existing 已提交文件（suppression 已移除但未格式化），不在本次 feature 切片内，归属 Milestone 7。真实 MinIO/Testcontainers 矩阵与完整 App production graph 留待 Docker 可用时复验。
  Date/Author: 2026-07-19 / Codex
...
本轮后续切片已经补齐 transaction-fenced evaluation、首次 formal baseline authority、paired regression、Rubrics-owned alert、Copy/DAG Decision、durable event/scheduled automation、heartbeat、recovery、Micrometer 和 ArchUnit 边界。验证证据包括 Task outcome 合同 2 项通过、架构/Judge/自动化/配置/指标定向测试 12 项通过、真实 MySQL Rubrics service 10 项通过、审查前完整 Surefire 报告 39 项全绿、独立 SpotBugs 零 finding，以及独立 Checkstyle 零 violation。双轴审查随后发现并修复 holdout 分母、UTF-8 边界、Decision evidence 伪装与平行 Subject dispatch；这些最后修复只完成 main/test compile，未在用户“只更新文档”之后继续运行测试。

专项计划仍不能整体标记完成。`IMAGE_RESULT` 尚缺真实 MinIO 的对象删除/替换/Pixel Budget 故障矩阵；`TASK_DECISION` 尚缺 owner-defined requirements/proposal，当前 trace 已按 decision revision 归属但仍不是 criterion-level span 精选；App 已通过干净静态 verify，但没有完整 production context/fail-fast context test。当前 Rubrics Checkstyle/SpotBugs suppression 已清零；`pixflow-web` 明确不在本轮授权范围，仍保留 Rubrics placeholder surface。父计划 Milestone 6 因真实依赖与完整上下文缺口保持未勾选。

2026-07-20 的后端收尾把 Eval trace 从“同会话任意 tool-call turn”收紧为“明确绑定当前 decision revision 的最新 50 turn 窗口”。App Proposal adapter 只把 owner hash 写入受控 metadata，Rubrics 再按结构化 `result.payloadHash` 精确匹配，消除了旧 Proposal、非 Proposal 工具和模型可控字段伪造证据的风险，且没有增加 Conversation implementation 依赖或兼容字段。干净 Rubrics/App verify 为零 Checkstyle/SpotBugs，48 项非容器 Rubrics 回归与 2 项 Proposal handler 测试全绿，当前工作树已无 Rubrics suppression；但 Docker daemon 未运行，真实 MySQL/MinIO 与完整 App production context 仍缺本轮证据，前端仍按用户要求不修改，因此专项计划与父计划 Milestone 6/7 均不标记完成。

按截至 2026-07-20 的最终实现与验收证据，Milestone 0、1、2、4 已完成。Milestone 3 仍缺真实 MinIO 对象删除、替换、损坏与 Pixel Budget 矩阵；Milestone 5 仍缺 Task/Conversation owner-defined requirements/proposal 和 criterion-level trace 精选；Milestone 6 仍缺完整 App production context 与真实依赖恢复矩阵；Milestone 7 因用户明确排除前端且全仓最终验收未执行而保持未完成。专项计划因此处于“核心深模块、可恢复运行与 Dataset/回归能力完成，真实对象/组合根/产品表面收尾未完成”的状态。
...
2026-07-20 / Codex: 收口 Milestone 3/5/6 后端实现。PublishedAssetReader 增加 immutable contentHash，ImageEvidencePackBuilder 同时校验发布 identity、大小和内容哈希，并对像素预算拒绝给出独立错误码；真实 MinIO 覆盖正常、删除、同长度替换、损坏和超预算。TaskOutcomeQuery 显式公开 confirmedProposal，TextEvidencePackBuilder 投影 PROPOSAL、DAG_SNAPSHOT 和 criterion-specific TRACE_SPAN。Rubrics 69 项（含真实 MySQL/MinIO）与 App 56 项测试全绿，Task outcome 合同 2 项全绿，静态门禁零 Checkstyle/SpotBugs。Milestone 7 的后端范围完成，前端清理按用户授权保留待办。

2026-07-20 / Codex: 补齐 IMAGE_RESULT 的 immutable provenance 与替换防护。`TaskOutcomeQuery.SuccessfulResultSnapshot` 公开已持久化的 producer provider/model，`ImageSubjectSnapshotResolver` 将其纳入 canonical hash；`ImageEvidencePackBuilder` 拒绝 published content size 或读取字节数与 Task 冻结 `bytesOut` 不一致的对象。新增单元测试覆盖同一 image identity 下内容替换，Image evidence 定向 5 项通过。真实 MinIO/Testcontainers、完整 App context 与前端清理未在本次授权内完成，因此各对应里程碑状态不变。

2026-07-20 / Codex: Docker 已恢复可用后运行真实 MySQL Rubrics schema、claim 与 service integration。schema 与 claim 报告通过；service integration 的单模块执行从本地仓库加载旧版 Task API，7 个测试报 `NoSuchMethodError`，必须改用完整依赖 reactor 复验。MySQL 和 MinIO 服务均健康，但仓库目前没有真实 MinIO image-evidence integration test，不能把服务可用性等同于 M3 的对象 I/O 覆盖。

2026-07-20 / Codex: 按已实现能力和验收证据回填里程碑状态：Milestone 1（模板/四态/Evidence/judge 核心）、Milestone 2（fresh-schema 与可恢复 Run Engine）、Milestone 4（Dataset/Gold Label/校准/baseline/paired regression/alert）标记完成；Milestone 3、5、6、7 因真实 MinIO、owner-defined Decision evidence、完整 App production context 和前端排除范围继续保持未完成。

2026-07-20 / Codex: 按当前 Task owner `payloadHash` 收紧 TASK_DECISION trace 归属。DAG/IMAGEGEN Proposal handler 在成功输出的受控 metadata 中记录 hash，Rubrics 只投影结构化 JSON 中精确匹配 `decisionRevision` 的 turn，并通过末两页合并精确截取最新 50 turn；补 revision 隔离、分页边界和 handler metadata 红绿测试。同步确认后续提交已清除 Rubrics suppression。Docker daemon 未运行，真实 MySQL/MinIO 与完整 App production context 未复验；按用户要求未修改前端。

2026-07-19 / Codex: 接通 Eval trace seam 并补 fail-fast/image 矩阵。rubrics pom 新增 `pixflow-eval` 依赖；新增 `TraceEvidenceProvider`/`DefaultTraceEvidenceProvider`，通过只读 `TraceQuery.listByConversation` 为 TASK_DECISION 投影有界 TRACE_SPAN 证据（按 turnNo 稳定排序、最多 50 turn、单 span 8 KiB UTF-8 安全截断、缺失/读取失败 best-effort 返回空不伪造 span）；`TextEvidencePackBuilder` 与 `RubricsAutoConfiguration` 接入，`TraceQuery` 作为生产必需依赖缺失即 fail fast。新增 `DefaultTraceEvidenceProviderTest`(7)、`RubricsAutoConfigurationTest`(2：judge 角色 fail fast + 禁止 `@ConditionalOnBean`)，并把 `ImageEvidencePackBuilderTest` 故障矩阵补到 4 项（缺失/损坏/替换/正常稳定 identity）。44 项单元测试零失败/错误/跳过、SpotBugs 零 finding、本次新增/修改文件零 Checkstyle violation。Task Decision owner-defined requirements/proposal 按 Decision Log 不伪造，待 Task/Conversation owner seam；真实 MinIO/Testcontainers 矩阵与完整 App production graph 受本机无 Docker 限制未跑；46 条 pre-existing Rubrics Checkstyle violation 按“记录不修复”保留。前端未触碰。

2026-07-19 / Codex: 继续执行后端专项计划，补齐不可变 Dataset/Gold Label/holdout 校准、首个 formal baseline、paired regression/alert、Copy 与真实 DAG Decision evidence、durable automation/recovery/heartbeat、Micrometer 和 ArchUnit。双轴审查后修复 holdout 分母、UTF-8 byte bound、伪 Proposal 与平行 Subject dispatch。按用户最新要求停止代码/测试动作，仅更新文档；记录最后修复只编译未复验、61 条 Checkstyle suppression 未处理、前端未触碰，以及 Eval trace/真实 MinIO/App context 剩余缺口。
