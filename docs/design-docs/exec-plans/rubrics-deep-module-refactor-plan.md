...
- [ ] 本轮后端收尾：Eval `TraceQuery`/`TraceReplay` seam 已接通（TASK_DECISION TRACE_SPAN 证据，2026-07-19 22:35）；Task Decision owner-defined requirements/proposal 仍待 Task/Conversation 提供 owner seam（按 Decision Log 不伪造，不新增 always-null 列）；image evidence 故障矩阵已补单元级（缺失/损坏/替换/正常），真实 MinIO Testcontainers 矩阵受本机无 Docker 限制未跑；Rubrics fail-fast 上下文守护已加（AI 角色 + 禁止 `@ConditionalOnBean`），完整 App production graph 仍需 Docker；按用户要求不修改前端；46 条 pre-existing Rubrics Checkstyle violation（suppression 已移除但未格式化）按“记录不修复”保留，归属 Milestone 7。
...
- Observation: 仅按 conversation 读取前 50 个含 tool call 的 turn 会把旧 Proposal trace 混入当前 Task Decision，且长会话的当前确认 turn 可能根本不在第一页。
  Evidence: 新增 `onlyUsesTraceForTheConfirmedDecisionRevision` 与 `readsTheLatestBoundedTraceWindowForLongConversations` 红灯测试分别暴露跨 revision 泄漏与第一页窗口缺失；实现按结构化 `payloadHash == decisionRevision` 过滤并读取最后一页后，`DefaultTraceEvidenceProviderTest` 9 项通过。
