...
- [ ] 阶段 1：把 Memory 请求切换为 canonical references，并通过 File public API 生成只读 recall filters。
- [ ] 阶段 2：补齐各数据源独立降级、确定性融合与真正的总 token budget。
...
本专项目前只完成调研与计划，没有修改生产实现，也没有运行测试。当前可复用成果是三方法只读 Vector runtime、只读 Memory 主体、canonical File public API、唯一 Product Visual Facts lookup tool，以及 Session Memory/PreCompact 两个合法 Agent hooks。剩余工作以 `Progress` 未完成项为准。
