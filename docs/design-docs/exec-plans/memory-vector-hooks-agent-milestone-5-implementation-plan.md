...
- [x] (2026-07-20) 阶段 0：复核工作树、owner-defined contracts 与数据库策略；采用开发期 fresh schema 直接删除 `sku_history.rubrics_score`，不增加 forward compatibility reader、双写或兼容视图。实施前基线提交为 `0d93921`，Memory/Agent 实现分别提交为 `4bc5532`、`4d7392a`。
...
- [x] (2026-07-20) 阶段 1 验收补强：增加可配置 PACKAGE 图片展开上限，覆盖 PACKAGE/SKU/original IMAGE/generated IMAGE/异常与有序去重；删除生产 `NoopRecallReferenceResolver`，File resolver/expander 改为构造期必需依赖。
- [x] (2026-07-20) 阶段 2：偏好、SKU history 与 insight 读取分别隔离失败；总预算按 `user_preferences -> sku_history -> analysis_insights` 顺序以完整 item 截断；预算、选择数量和降级状态进入 recall trace。
- [x] (2026-07-20) 阶段 2 验收补强：统一 `asOf`，FULLTEXT/偏好/SKU history 使用稳定 tie-breaker，SKU history 以窗口函数落实 per-SKU 上限；覆盖 budget=0、Unicode、超长项、逐源/全源失败和安全降级原因；真实 MySQL/Qdrant 验证零写快照。
- [x] (2026-07-20) 阶段 3：Agent 生成 bounded/sanitized recall snapshot，Loop 每回合只转投一次且 COMMITTED/ABORTED/CANCELLED 三终态均保留；Prompt fingerprint 不包含诊断 trace；App mock-model 两轮组合同时证明自动 Memory 注入和显式 Product Visual Facts 工具结果。
- [x] (2026-07-20) 阶段 4：固化 Hooks、Session Memory、Rubrics 与 Vector 的无写边界，删除 Memory 生产 Noop、`SkuHistory.rubricsScore` 实体残留及旧数据库合同。
- [ ] 阶段 5（已完成：File owner 查询层 bounded expansion、App 生产接缝组合、MySQL/Qdrant 成功路径与零写、真实 Qdrant 错维度/错 distance、模块静态门禁；剩余：真实依赖停机故障矩阵、完整 App production context，以及被当前 Hooks/Loop runtime-scope 合同不一致阻断的全仓 `mvn verify`）。
...
- Observation: canonical identity 与 Agent adaptation 已拆成两个独立提交，当前工作树中的 Memory/Agent 实现已提交，不再是未提交补丁。
  Evidence: `4bc5532 feat(memory): resolve canonical references for recall` 与 `4d7392a feat(agent): pass canonical references to memory recall`；2026-07-20 复核时 `git status --short` 只显示范围外 Rubrics 计划修改和 `.scratch/`。

- Observation: 当前 `MemoryAutoConfiguration` 在 File resolver/expander 任一缺失时装配 `NoopRecallReferenceResolver`，这与阶段 5 的生产 fail-fast 目标仍有差距。
  Evidence: `recallReferenceResolver(...)` 使用两个 `ObjectProvider`，缺失时返回 `NoopRecallReferenceResolver`。该 fallback 可服务显式测试 slice，但生产 profile 必须在阶段 5 收口为明确依赖，不能静默丢失 canonical filter。

- Observation: 首轮真实依赖测试没有形成可接受证据，不能据此标记故障矩阵完成。
  Evidence: `mvn -pl pixflow-module-memory,pixflow-agent -am test` 在 120 秒命令上限内超时且没有完成报告；后续仅完成非容器单元测试和定向 compile。

- Observation: Memory 集成测试直接启用全部 classpath AutoConfiguration 会被 File/Permission 等无关生产图依赖干扰。
  Evidence: fail-fast 后首次容器测试先因重复 File resolver，再因 Permission proof 缺失而无法启动；测试最终显式排除 File/Permission AutoConfiguration，并为 Memory seam 提供 fake File owner API，真实 MySQL/Qdrant 路径随后零 skip 通过。

- Observation: 全仓测试 reactor 当前被本任务范围外的 Rubrics/Task 合同修改阻断，不能通过修改 Milestone 5 代码规避。
  Evidence: `mvn -pl pixflow-app -am ... test` 在 `pixflow-module-rubrics:testCompile` 失败，四处测试仍调用已新增字段的 `TaskDecisionSubject` / `ConfirmedDecisionSnapshot` 旧构造器；这些文件与 `.scratch/`、Rubrics 专项计划属于用户并行改动。本任务改用直接模块命令验证 Memory、Loop、Agent、App，并保持父计划未完成。

- Observation: 最新一次全仓 clean reactor 已越过此前的 Rubrics/Task 编译点，但暴露出本任务未修改的 Hooks/Loop runtime-scope 源码合同与测试期望不一致。
  Evidence: 2026-07-20 执行 `mvn verify` 时，前 13 个模块成功，`pixflow-loop` 的 `RuntimeScopeTranslatorTest.subagentEvalMapsToSubagentHooks` 期望 `sub_agent` 但得到 `subagent`，`RuntimeStateTest.runtimeScopeDefaultsToMain` 因默认 scope 为 `null` 抛出 NPE；相关四个源码/测试文件相对 `HEAD` 均无 diff。本任务不扩大范围修复该并行合同变化。
...
- Decision: Memory 直接依赖 `pixflow-module-file` 的公开 `AssetReferenceResolver` 与 bounded `AssetReferenceExpander.expand(referenceKeys, use, maxImages)`，不复制 File DTO，不建立 App-owned 同义端口。
...
截至提交 `4d7392a`，本专项已交付第一批可编译实现：canonical Memory 请求与 Agent adaptation、File owner best-effort 解析、三 section 源级降级、跨 section token budget、Agent 门面异常时的 degraded 空 context，以及 fresh schema 中 `sku_history.rubrics_score` 的删除。旧 `MemoryAttachment` 和 Memory request 的 package/task/SKU 平行身份已从生产源码删除，没有保留兼容构造器。

已完成的精确验证为：`mvn -pl pixflow-module-memory,pixflow-agent -am -DskipTests compile` 成功；`RecallPlannerTest` 2 项通过；通过完整依赖 reactor 运行的 `MemoryRecallPlannerTest` 与 `AgentOrchestratorTest` 共 5 项通过；`git diff --check` 无 whitespace error；生产源码与当前 Memory schema 不再命中 `rubrics_score`。这些证据证明第一批合同切换和基本降级路径可编译、可单测，但不足以证明整个 Milestone 5 完成。

尚未完成的关键闭环是 resolver 全矩阵与展开上限、统一 `asOf`/确定性与 write-spy、Loop/Eval recall trace、Prompt/视觉事实组合测试、Hooks/架构负向守护、生产 fail-fast 装配、真实 MySQL/Qdrant 故障矩阵和全仓 `mvn verify`。真实依赖测试曾在 120 秒命令上限内超时，未得到完成报告，因此阶段 5 和父计划 Milestone 5 均保持未完成。

2026-07-20 后续切片已完成 PACKAGE 上限和生产 fail-fast、统一 `asOf`、稳定 Prompt fingerprint、bounded recall trace、Memory/Vision 只读合同测试，以及真实 MySQL/Qdrant 的成功路径零写验证。定向单元测试累计新增/扩展 24 项均通过；容器测试 1 项零 skip 通过。完整故障矩阵、Loop 三终态组合、App mock-model 组合和全仓验证仍未完成，因此整体里程碑继续保持未完成。

2026-07-20 收尾切片补齐 SKU history 的 MySQL 8 窗口查询，以 `(created_at DESC, id ASC)` 在每个 SKU 分区内独立限额，并从实体删除已不在 fresh schema 中的 `rubricsScore`。Memory 逐源/全源故障测试共 4 项、混合 insight 四组合测试 6 项和 resolver/ranker/装配测试共 4 项通过；真实 MySQL 8.0 + Qdrant 1.18.2 集成测试扩展为 2 项，均零 skip，并继续证明召回前后 MySQL/Qdrant 不变。

Loop 的 COMMITTED、ABORTED、CANCELLED 三终态定向回归与 `TraceFanout` 共 17 项通过。App mock-model 组合测试在第一轮模型请求中观察到 Memory Context 和唯一视觉事实工具 schema，执行 `get_product_visual_facts` 后第二轮观察到脱敏 tool result；Memory planner 仅调用一次，recall/tool trace 分别存在。App 架构与组合测试共 7 项通过。`pixflow-module-memory`、`pixflow-loop`、`pixflow-agent`、`pixflow-app` 分别执行 `-DskipTests verify`，Checkstyle 与 SpotBugs 均为零发现。

阶段 5 仍未完成：现有 `QdrantVectorSearchIntegrationTest` 以真实 Qdrant 覆盖错维度和错 distance，但真实 Qdrant/MySQL 停机仍只有 adapter/融合层故障注入，尚未形成完整 Testcontainers 停机矩阵；全仓 test reactor 又被范围外 Rubrics/Task 旧构造器测试阻断。因此不更新父计划 Milestone 5 完成状态，也不把本计划整体标为完成。

2026-07-20 最终刷新验证：`MemoryVisionAgentCompositionTest`、`ProposalOwnershipArchitectureTest`、`AppArchitectureTest` 共 8 项通过；五个受影响模块在同一 reactor 中执行 `-DskipTests verify` 均成功，Checkstyle/SpotBugs 零发现；真实 MySQL 8.0 + Qdrant 1.18.2 的 `MemoryMySqlQdrantIntegrationTest` 与 `MemoryReadOnlyContractTest` 共 4 项通过、零 skip。全仓 `mvn verify` 运行至第 14/30 模块后，在未被本任务修改的两个 Hooks/Loop runtime-scope 合同测试失败；因此阶段 5 继续保持未完成，且当前全仓阻塞证据以该次 clean reactor 为准。
...
`pixflow-module-file` 拥有 canonical Asset Reference 解析。公开 `AssetReferenceResolver.resolve(referenceKey, AssetUse)` 返回不含对象位置的 `ResolvedAssetReference`；`AssetReferenceExpander.expand(referenceKeys, AssetUse, maxImages)` 在 owner 查询层限界并把 PACKAGE/SKU 展开为当前图片视图。Memory 只利用这些结果生成 SKU/category 等 read filters，不持有 File mapper/entity/object key。
...
2026-07-20 / Codex: 根据已提交实现与测试证据重新校准 living sections。记录 `4bc5532`/`4d7392a`，把阶段 0、阶段 1 和阶段 2 已落地的主项标为完成；未完成的验证与生产收口拆成独立 checklist 条目。新增 `NoopRecallReferenceResolver` 与生产 fail-fast 的差距、真实依赖测试超时事实，并在 Outcomes 中写入精确命令、测试数量和下一停止点。本次只更新计划文档，不修改生产代码，也不更新父计划 Milestone 5。

2026-07-20 / Codex: 实施后续验收切片。Memory 删除生产 Noop reader/resolver，File 与 MySQL owner 依赖改为 fail fast；PACKAGE 展开限界，召回统一 `asOf`，FULLTEXT 与 Prompt fingerprint 稳定；Agent 将无正文的 bounded recall snapshot 写入 RuntimeState，Loop 每回合只转投一次 Eval；新增 Memory 只读和 Product Visual Facts descriptor 合同测试。真实 MySQL 8.0 + Qdrant 1.18.2 集成测试零 skip 通过并证明 MySQL/Qdrant 前后快照不变。完整故障矩阵与 App 组合仍待完成。

2026-07-20 / Codex: 完成阶段 2/3/4 收尾。SKU history 改为 MySQL 8 窗口函数实现 per-SKU 限额并删除实体旧 `rubricsScore`；三类 Memory source 的降级 trace 统一给出安全原因。Loop 三终态保留 recall trace，App mock-model 两轮组合证明 Memory 与显式视觉事实工具互不调用、分别可观测。模块定向测试、App 架构测试、真实 MySQL/Qdrant 两项容器测试和四个变更模块静态门禁通过。真实停机/错误 collection 矩阵及全仓测试仍未完成，后者被范围外 Rubrics/Task 并行修改的测试编译错误阻断，故父计划状态保持未完成。

2026-07-20 / Codex: 根据最终 Standards/Spec 收尾补强 File owner-side bounded expansion、取消路径 recall trace、App 真实 Memory planner/assembler 组合与 fail-fast context 测试，并刷新验证证据。复审进一步发现并修复 owner 查询中无效图片提前消耗 `LIMIT`、非空 PACKAGE 全部无 SKU 时误报 resolved、Eval snapshot 丢失 reference degradation 分类三项问题；对应回归测试和 File/Memory/Agent 静态门禁通过。App 定向 8 项、Memory 真实容器 4 项和五模块静态门禁均通过；全仓 `mvn verify` 的最新阻塞更新为未修改的 Hooks/Loop runtime-scope 合同测试不一致。真实停机故障矩阵和完整 App production context 仍未完成，不更新父计划 Milestone 5。
