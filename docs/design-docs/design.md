...
│  │  自动记忆召回 + 动态 Prompt 组装                            │    │
│  │  Tool Registry(Agent级动作): query_commerce_data /         │    │
│  │     compile_dag / submit_dag(需令牌) / run_vision_subagent │    │
│  │     / run_imagegen_subagent                               │    │
│  └───────┬─────────────┬──────────────┬───────────┬─────────┘    │
│          │             │              │           │              │
│  ┌───────▼──────┐ ┌────▼─────┐  ┌─────▼────┐ ┌────▼─────────┐    │
...
|---|---|---|
| 例 | `query_commerce_data`、`compile_dag`、`submit_dag`、`run_vision_subagent`、`run_imagegen_subagent` | `remove_bg`、`set_background`、`resize`、`compress`、`watermark`、`convert_format`、`generate_copy`、`compose_group` |
...
| `message` | id, conversation_id, role, content, attached_package_id, task_id, created_at | 消息(transcript);由 `harness/session` 唯一写入(实现 context 的 `TranscriptPort`),`module/conversation` 只读 |
