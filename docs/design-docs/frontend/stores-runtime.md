...
Materials, Outputs, search forms, paging cursors, gallery selection, previews, and rename dialogs are page state. They reload from backend facts after route recreation.
...
Stores expose state and intent methods. Vue components do not open transports, expand reference keys, invent idempotency keys, or orchestrate retries.

## Persistence policy

Allowed local persistence is limited to non-sensitive panel preferences and small upload-session hints. Access tokens, message queues, Proposal cards, file bytes, chunks, pre-signed URLs, and task execution state are never persisted by the frontend.
