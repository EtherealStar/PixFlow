...
public enum ModelRole {
    PRIMARY_CHAT,   // 主 Agent 决策 / compile_dag / rubrics 文本评估
...
- **`toolChoice` 默认 `AUTO`**：本期不做强制 JSON / structured-output；`compile_dag` 就是模型自主决定调用的一个工具。
...
> 纯工具调用轮（如 `compile_dag`、多数「决策即行动」轮）整轮无可见 token，`emittedDownstream` 始终为 `false` → **失败可全程透明重试**，等价于自动缓冲。带可见文本的轮才可能走「已发射」分支。
...
| `module/dag` | 经 `ChatModelClient` 实现 `compile_dag`（NL→DAG JSON），`toolChoice=AUTO` |
...
- **强制 structured-output / JSON mode**：本期 `toolChoice=AUTO`，模型自行决定调工具；如未来 `compile_dag` 需要更强约束再评估。
