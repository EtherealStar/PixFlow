...
> 范围：对模型供应商的**供应商无关调用抽象**——文本 chat（同步 + 真流式）、多模态、生图（源图重绘）、嵌入、重排，加模型路由、韧性重试、错误源头构造、全局并发封顶、Redis 出站额度、用量与可观测。
...
- [九、全局调用治理：并发许可 + Redis 加权令牌桶](#九全局调用治理并发许可--redis-加权令牌桶)
...
4. **模型型号全部配置化**。逻辑角色（`PRIMARY_CHAT`/`VISION`/`RUBRICS_JUDGE_TEXT`/`RUBRICS_JUDGE_VISION`/`IMAGEGEN`/`EMBEDDING`/`RERANK`）映射到 `provider + model + 默认参数`，由配置承载；换模型只动 YAML（见 [§四](#四模型路由逻辑角色--供应商型号)）。Rubrics judge 角色与产出角色分离并锁定 evaluator version，不能隐式回退到 `PRIMARY_CHAT`。
5. **错误源头即构造**。按 `common.md §10` 的混合策略，本地 Redis 桶拒绝、供应商限流、网络/供应商/上下文超限错误**在源头即构造**带 `category` + `retryAfter` 的 `PixFlowException`，让重试器尽早拿到 retryable 信息。本地拒绝与 provider 429 必须使用不同 errorCode。
...
8. **向量库无感**。infra/ai 只做「查询文本 → 向量」的嵌入与「query+候选 → 重排」，Qdrant 只读检索归 `infra/vector`；不使用 Spring AI 把嵌入与存储耦合的 `VectorStore`（见 [§十一](#十一嵌入与重排边界)）。
...
| 模型调用的重试/退避、额度准入、错误归一化 | **infra/ai** | 与 attempt 和 retryable 判定强相关；Redis 实现经 SPI 注入 |
...
│   ├── ConcurrencyGuard.java        # 每 attempt 经 GlobalConcurrencyLimiter 取许可
│   └── ModelQuotaGuard.java         # 每 attempt 经 ModelQuotaLimiter 加权消费
├── spi/
│   ├── GlobalConcurrencyLimiter.java# SPI 倒置：组合根接 DistributedSemaphore
│   └── ModelQuotaLimiter.java       # SPI 倒置：组合根接 DistributedTokenBucket
├── error/
...
infra/ai ──► (SPI) GlobalConcurrencyLimiter / ModelQuotaLimiter
pixflow-app ──► infra/ai + infra/cache（组合根提供 Redis adapters）
...
public enum ModelRole {
    PRIMARY_CHAT,   // 主 Agent 决策 / DAG 提案撰写
    VISION,         // 视觉理解子 Agent（Qwen-VL 类）
    RUBRICS_JUDGE_TEXT,   // Rubrics 文本/trace criterion judge
    RUBRICS_JUDGE_VISION, // Rubrics 图片 criterion judge
    IMAGEGEN,       // 生图子 Agent（通义万相类，源图重绘）
    EMBEDDING,      // 只读召回的查询文本向量化
    RERANK          // 记忆召回重排
}
```
...
public record ChatRequest(
        ModelRole role,                 // 主路径默认 PRIMARY_CHAT；Rubrics 必须指定 RUBRICS_JUDGE_TEXT
...
### 5.2 VisionModelClient（多模态商品事实 / Rubrics vision judge）
...
public record VisionRequest(
        ModelRole role,                 // VISION 或 RUBRICS_JUDGE_VISION
        List<ChatMessage> messages,       // 文本 part + 图片 part（图片内容由调用方解析好）
        List<ToolSchema> toolSchemas,     // 商品视觉事实使用唯一强制工具；Rubrics 可为空
        ToolChoice toolChoice,
        ChatOptions options,
        AttemptBudget attemptBudget       // 可空；非空时限制本请求链的实际 provider attempt
) {}
```

- `module/vision` 通过现有 `DefaultVisionModelClient -> ChatModelClient -> OpenAI-compatible chat completions` 路径提交图片，不新增 VLLM 专用 provider。
- 商品视觉分析只声明 `submit_product_visual_facts`，并强制 tool choice；工具调用是结构化输出信封，不进入 Agent `ToolExecutor`。
- `AttemptBudget` 使调用方能把传输重试与结构修复纳入同一持久预算。每次真实 provider attempt 仍在 `infra/ai` 内获取并发许可、消费 Redis 出站额度，并在发出前通知预算 owner；不得让全局重试在预算之外发生，再把整次 `call` 只计为一个业务 attempt。
- Rubrics 的视觉 judge 保持自己的 role、prompt 与多次 rollout 语义，不复用商品事实工具 Schema。

`ChatMessage` 的图片 part 承载 **图片内容本身**（bytes / data-uri / 预签名 URL），不承载 MinIO 逻辑 key——infra/ai 不依赖 storage（见 [§十](#十多模态与生图边界)）。Chat 与 Vision 接口**分离**，但底层共享 `ChatMessage` 模型与同一套调用/累积实现。`RUBRICS_JUDGE_*` 角色的 resolved provider/model/options 由调用方写入 evaluator version；infra/ai 仍只负责单次调用，不理解 criterion、Evidence Pack 或投票。
...
- Proposal 一次确认与权限硬拦截在 Conversation/permission，和本模块无关；infra/ai 只执行已被上游放行的单次模型调用。
...
- **并发许可与额度消费位于 `attemptSupplier` 内**：每次进入真实 provider attempt，先获取全局并发许可，再调用 `ModelQuotaGuard.tryConsume`，最后发起 HTTP；失败后的退避不持有并发许可。不能把 quota guard 包在 `ModelRetryRunner.run(...)` 外层，否则 retry 会绕过计费。
- **本地 Redis 桶拒绝发生在首个 delta 前**：映射为 `MODEL_LOCAL_RATE_LIMITED`（RATE_LIMIT/RETRY）并携带 bucket decision 的 `retryAfter`；runner 按现有策略透明等待/重试。Redis/Lua 故障映射为 `MODEL_QUOTA_UNAVAILABLE`（DEPENDENCY/RETRY）并 fail-closed。
...
- **退避策略**（`RetryPolicy`）：`maxRetries` 默认 3、`baseDelay` 0.5s、`maxDelay` 32s、`jitterRatio` 0.25；`delay = min(base·2^(n-1), cap) + jitter`；供应商给 `Retry-After` 时优先采用。商品视觉请求还受 request-scoped 的 3 次**实际 provider attempt** 总预算约束，传输重试不得突破该预算。
...
| Redis 本地策略桶额度不足 | `MODEL_LOCAL_RATE_LIMITED` | RATE_LIMIT | RETRY | 使用 quota decision |
| Redis/Lua 令牌桶不可用 | `MODEL_QUOTA_UNAVAILABLE` | DEPENDENCY | RETRY | 基础依赖退避 |
...
## 九、全局调用治理：并发许可 + Redis 加权令牌桶

`infra/ai` 保持只依赖 `common`，不直接依赖 cache。AI 自己拥有两个小接口，`pixflow-app` 作为组合根提供 Redis adapters：并发接口落到 `DistributedSemaphore`，额度接口落到 `DistributedTokenBucket`。
...
public interface ModelQuotaLimiter {
    ModelQuotaDecision tryConsume(ModelQuotaRequest request);
}

public record ModelQuotaRequest(
        ModelRole role,
        String provider,
        String quotaGroup,
        long cost) {}

public record ModelQuotaDecision(
        boolean allowed,
        long remaining,
        Duration retryAfter) {}
```
...
- `ModelQuotaLimiter` 是 AI 调用方看到的唯一额度 seam；Redis key、Lua、capacity/refill policy 和指标都藏在组合根 adapter + cache 实现后面。单元测试使用 deterministic fake adapter。
- 每个 role 配置 `quota-group` 与 `cost-per-attempt`。adapter 构造 `bucket:ai:{provider}:{quotaGroup}`，允许多个角色共享同一供应商配额；默认可令 `quotaGroup=role`。V1 的 cost 是**请求权重**，不是实际 prompt/completion token 或供应商账单。
- 每个真实 attempt 依次执行 `ConcurrencyGuard.acquire → ModelQuotaGuard.tryConsume → provider HTTP`。quota 拒绝立即释放并发许可；HTTP 一旦开始，无论结果成功、超时还是 5xx，本次令牌都不返还。
- 额度不足是正常控制流，返回 `retryAfter`；Redis 故障是依赖错误，fail-closed。生产配置缺少任一 concurrency/quota adapter 时应用启动失败；只有明确的 test profile 可注入 noop/fake，不能静默无限制运行。
- provider 429 与本地 bucket 拒绝都可重试，但使用不同 errorCode。CircuitBreaker（若后续为 AI 引入）不得把两类 rate-limit 计为供应商可用性故障。
- 用户级 HTTP 防滥用、租户预算、RPM/TPM 双维度和实际账单核算不在本期；没有 owner/tenant 进入模型请求时，不宣称按用户限流。

```mermaid
flowchart LR
  AI["infra/ai attempt guards"] --> Concurrency["GlobalConcurrencyLimiter"]
  AI --> Quota["ModelQuotaLimiter"]
  App["pixflow-app composition root"] -.adapter.-> Concurrency
  App -.adapter.-> Quota
  App --> Cache["infra/cache semaphore + token bucket"]
  AI --> Call["provider attempt"]
```
...
- **HITL 与本模块无关**：生图/重跑的 Proposal 确认与权限硬拦截在上游；infra/ai 不参与是否放行的判断。
...
pixflow:
  ai:
    ...
    concurrency:                         # 全局并发上限（经 GlobalConcurrencyLimiter 落到 cache 信号量）
      ...
      vision: 8
      primary-chat: 16
    quota:                               # provider attempt 级请求权重；不是实际 token 计费
      roles:
        primary-chat: { quota-group: chat, capacity: 60, refill-tokens: 60, refill-period: 1m, cost-per-attempt: 1 }
        vision:       { quota-group: vision, capacity: 20, refill-tokens: 10, refill-period: 1m, cost-per-attempt: 3 }
        imagegen:     { quota-group: imagegen, capacity: 10, refill-tokens: 2, refill-period: 1m, cost-per-attempt: 5 }
        embedding:    { quota-group: embedding, capacity: 100, refill-tokens: 100, refill-period: 1m, cost-per-attempt: 1 }
        rerank:       { quota-group: rerank, capacity: 60, refill-tokens: 60, refill-period: 1m, cost-per-attempt: 1 }
```
...
- quota 数值是安全默认示例，部署时必须按供应商配额校准；多个 role 只有在确实共享同一供应商额度时才复用 `quota-group`。
...
- **额度指标**：`pixflow.ai.quota{role, provider, result=allowed|rejected|error}`；remaining 只做低基数 role/provider 聚合或日志采样，不把 conversationId/userId 放进 tag。
...
| `infra/vector` | 分工：ai 生成查询向量，vector 只读检索；不使用 Spring AI 耦合 `VectorStore` |
...
- **每 attempt 额度**：两次 provider attempt 必须消费两次权重；额度拒绝不发 HTTP、保留 `retryAfter`；Redis 故障 fail-closed；退避期间不持有 concurrency permit。
- **SPI 装配**：生产 profile 缺少 quota adapter 启动失败；test profile 可注入 fake/noop；adapter contract 以 Testcontainers Redis 验证跨实例共享额度。
...
- **并发 SPI**：注入 fake `GlobalConcurrencyLimiter` 断言每 attempt 取/释许可；生产 profile 未注入时装配失败，test profile 显式 noop 才允许不限并发。
...
- **真实 token 预算 / TPM / 每日费用 / 用户级配额**：本期只有 provider attempt 请求权重桶；真实 token 用量只做响应后观测，不回填令牌桶。
...
2026-07-12 / Codex: 为离线 Rubrics 引入独立、版本锁定的 `RUBRICS_JUDGE_TEXT` / `RUBRICS_JUDGE_VISION` 逻辑角色；`PRIMARY_CHAT` 不再兼任 judge，`VisionRequest` 显式携带 role。infra/ai 仍只负责单次 provider 调用，不理解 criterion、Evidence Pack 或投票。
...
2026-07-11 / Codex: 新增 `ModelQuotaLimiter` seam，通过 `pixflow-app` adapter 复用 `infra/cache` Redis Lua 加权令牌桶；固定 concurrency/quota 位于每个 `ModelRetryRunner` attempt 内，生产缺失 adapter 启动失败，区分本地桶拒绝、provider 429 与 Redis 故障。明确 V1 只做请求权重，不宣称真实 token 计费或用户级限流。
