...
1. **领域无关**。infra/mq 不认识「任务」「素材包」「视觉分析」。它只提供 `MessagePublisher` / 消费容器 / 目的地声明 / 重试-DLQ 机制等通用能力；业务 topic、tag、consumer group、消息体、失败判定全部由上层模块声明与实现。
2. **至少一次投递（at-least-once）**。可靠投递 + consumer group 消费确认，保证消息不轻易丢；重复投递交由消费侧幂等消化（`process_result` checkpoint、MySQL 唯一约束/状态机、Redisson 锁），infra/mq 不自建幂等表。
3. **可靠性优先于吞吐**。本系统消息量天然低：任务消息是一条任务级消息，上传解压与视觉分析入口是一条包级消息，视觉执行是一条 SKU 工作项消息。默认同步发送、显式结果检查、较小并发、业务幂等兜底，不为吞吐牺牲可恢复性。
...
| 上传解压、视觉分析作业/工作项幂等接管 | **module/file / module/vision** | 包级入口与 SKU 工作项流程 |
...
| Tag | `MessageDestination.tag` | 同一 topic 下的消息类型，例如 `TASK_EXECUTE`、`PACKAGE_EXTRACT`、`PACKAGE_VISUAL_ANALYSIS_REQUESTED` |
...
| 商品视觉分析入口 | `pixflow-vision` | `PACKAGE_VISUAL_ANALYSIS_REQUESTED` | `pixflow-vision-analysis` | `package:{packageId}` |
| SKU 视觉分析工作项 | `pixflow-vision` | `VISION_ANALYZE_ITEM` | `pixflow-vision-analysis` | `vision-item:{itemId}` |
...
- **业务幂等**：file 以 `packageId + originalPath` 跳过已解压图片；vision 的包级入口以 `packageId` 唯一作业吸收重复消息，SKU 消费以 `itemId`、状态与 `run_epoch` fencing 防止重复提交事实。
...
| `module/vision` | file 解压完成后发布 `PACKAGE_VISUAL_ANALYSIS_REQUESTED`；vision 创建包级 job、按 SKU 建 item 并发布 `VISION_ANALYZE_ITEM`。MQ 至少一次分发，MySQL item/epoch 是执行事实源，不再写 `asset_copy` |
...
5. `module/vision.md` 使用 `pixflow-vision` topic、`PACKAGE_VISUAL_ANALYSIS_REQUESTED` / `VISION_ANALYZE_ITEM` tag 与 `pixflow-vision-analysis` consumer group，承载包级入口和可恢复的 SKU 视觉分析工作项；不再承载文案富化。
...
- **业务幂等**：重复投递 `taskId` 不重复执行已完成单元；重复投递视觉分析 `packageId` 不重复创建包级作业，重复投递 `itemId` 不重复提交当前事实快照。
...
- 顺序消息（当前任务、解压、视觉分析都靠业务幂等，不要求严格顺序）。
