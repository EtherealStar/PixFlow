...
| 工具空间 | 开放（含 bash 任意命令） | 固定少量动作；Agent 工具层只提交提案，真实副作用在确认 REST 边界受控 |
...
| 拦截点 | 机制 |
|---|---|
| 生成建议后必须等用户确认，禁止自动执行 | `submit_image_plan` / `submit_imagegen_plan` 只产生待确认提案；真实执行由确认 REST 端点携确认令牌触发 |
| 大批量重处理二次确认（超阈值张数） | 超阈值的真实执行在确认 REST 边界要求 `BULK` 级令牌 |
| 生图/重跑前用户确认 | 生图提案确认后执行；重跑在确定性路径上复用同一确认 REST 令牌机制 |
...
**为什么三态而非两态**：在确认 REST 边界，真实执行无令牌在 PixFlow 里**绝大多数是正常时序**（Agent 已提交提案、用户还没点确认），不是违规。用 `DENY`/错误表达它会让日志被大量「假错误」污染，前端也无法区分「你要确认」和「你被拒了」。`CONFIRM_REQUIRED` 把 HITL 暂停建模为一等公民状态。注意：`harness/tools` 的 Agent 工具集零令牌，正常不应从工具执行管线收到 `CONFIRM_REQUIRED`；若收到，tools 必须按 fail-closed 处理。
...
public record PermissionDecision(
    ...
    String subject,                 // permission_subject：被判定的动作主体（如 "submit_image_plan" 或 "image_plan_confirm"）
...
public record PermissionSubject(
    String toolName,                         // Agent 级动作名或确认 REST 动作名，如 submit_image_plan / image_plan_confirm
    boolean readOnly,                        // 本次调用是否只读；由 tools 分类后适配
    ConfirmationAction confirmationAction,   // Agent 工具层普通动作为空；确认 REST 边界执行 DAG=SUBMIT_DAG，生图=IMAGEGEN
    String conversationId,
    String packageId,
...
**反例（不采用）**：把令牌作为 `submit_image_plan` 或任何 Agent 工具的 schema 字段由 LLM 填写。即使服务端验签能挡住「捏造」，也挡不住 LLM「重放」上下文里见过的旧令牌。
...
sequenceDiagram
    ...
    Note over Loop: 第 N 轮：Agent 调 submit_image_plan 产出待确认提案
    Loop->>U: SSE 返回建议 + pendingPlanId
    U->>API: POST /conversation/{id}/confirm (用户点确认)
    API->>API: 服务端按已校验 DAG 算 payloadHash + 数实际 count
    API->>Store: save(tokenId, claims, TTL)
    API->>Perm: evaluate(subject, context)
    Perm->>Store: verifyAndConsume(tokenId)  原子读取+删除
    Store-->>Perm: claims（校验 payloadHash/count/exp/action）
    Perm-->>API: ALLOW（令牌已消费）
    API->>API: 创建任务 / 触发生图执行
```

`submit_image_plan` / `submit_imagegen_plan` 的工具 schema **不含任何 token 字段**。它们只创建待确认提案；真实执行不通过 Agent 工具再次发起。
...
> **`ConfirmationAction` 只有两个值 `SUBMIT_DAG` / `IMAGEGEN`**（决策 Q4）：「重跑」不是独立动作，它在确定性路径上就是「确认执行一次（可能裁剪过的）DAG」，复用 `SUBMIT_DAG` 的令牌闸门即可；`payloadHash` 以「实际要执行的 DAG/支路集合」计算，天然覆盖「只重跑失败支路」的场景。

### 5.3 校验：verifyAndConsume

确认 REST 边界触发 DAG/imagegen 真实执行前，`ConfirmationTokenService.verifyAndConsume` 读取 `PermissionSubject` 中的真实执行事实，依次校验：
...
flowchart TD
  ...
  A3 -->|是| Deny
  A3 -->|否| B1{"confirmationAction ∈ {SUBMIT_DAG, IMAGEGEN}?"}
  B1 -->|否| C1["普通动作（search/read/agent/plan/plan_exit/submit_*_plan 提案）"]
...
**阶段 B（副作用动作令牌闸门）**：确认 REST 边界的真实 DAG 执行 / 生图执行：
- 无令牌 → `CONFIRM_REQUIRED`（正常暂停，非错误）。
- 有令牌但校验失败 → `DENY`。
- 校验通过 → `ALLOW`（消费令牌）。

**阶段 C（普通动作）**：`search` / `read` / `agent(type=vision|explore)` / `plan` / `plan_exit` / `submit_image_plan` / `submit_imagegen_plan` 默认按只读或提案动作放行；命中配置 deny/disable 规则才拒绝。Agent 工具层零令牌，正常不返回 `CONFIRM_REQUIRED`。

> PixFlow 工具集固定且小，阶段 C 远比 OneCode 轻——只读动作和提案动作几乎全是默认放行，真正受控的是阶段 B 的确认 REST 真实执行动作。
...
- **子 Agent 硬约束**：只读 vision/explore 子 Agent 调 `submit_image_plan` 或 `submit_imagegen_plan` → `SUBAGENT_FORBIDDEN_ACTION`；递归 `agent` 禁用。
- **令牌与 LLM 隔离**：断言 `submit_image_plan` / `submit_imagegen_plan` 的 schema 不含 token 字段；确认 REST 边界的 `evaluate` 不读任何 LLM 可见 `tool_input` 中的令牌状字段。
